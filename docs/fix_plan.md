...
- Attempts History:
  ...
  * [2025-10-10] Attempt #12 — Result: ✅ success (Phase D2 fluence parity fix). Root cause: **NOT** in PyTorch simulator core code, but in `debug_pixel_trace.py` script itself. Fixed TWO bugs: (1) Changed fluence formula from circular beam area `π·(beamsize/2)²` to square beam area `beamsize²` per spec-a-core.md §5.2 line 517; (2) Changed flux default from 1e12 to 0.0 to match C code defaults. Post-fix trace for pixel (1792,2048) shows machine-precision parity: PyTorch fluence=1.259320152862271e+29, C fluence=1.25932015286227e+29, rel_err=8.38e-16 (target ≤1e-3). PyTorch simulator code in `src/nanobrag_torch/config.py` lines 526-545 was CORRECT all along. Pytest collection verified (692 tests, no regressions). Artifacts: `reports/2026-01-vectorization-parity/phase_d/20251010T064644Z/{fluence_parity.md,py_traces_post_fix/pixel_1792_2048.log,commands.txt,env/}`. Next: Phase D3 (F_latt ~100× normalization).
...
- Next Actions (2025-10-10 refresh — Phase C3 complete, ready for implementation):
  ...
  6. **Phase D1 — Fix H1 (scattering_vec unit conversion)** — Update PyTorch to emit `scattering_vec_A_inv` in **m⁻¹** (match spec/TRACE_C). Follow `plans/active/vectorization-parity-regression.md` Phase D1 guidance; rerun `debug_pixel_trace.py` for pixel (1792,2048) and log before/after values in `diff_scattering_vec.md` (tolerance ≤1e-6 relative).
  7. **Phase D2 — Fix H2 (fluence calculation)** — Reconcile BeamConfig/simulator fluence with `specs/spec-a-core.md` §5.2 so TRACE_PY `fluence` aligns with C within 1e-3. Document derivation in `fluence_parity.md` per plan Phase D2.
