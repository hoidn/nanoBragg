...
- Priority: High
- Status: in_progress (Phase B reproduction outstanding)
...
- Next Actions (2026-01-01 refresh — Phase B scope now blocking parity triage):
  1. **Phase B1** — Rerun the 4096² benchmark on current HEAD using `NB_C_BIN=./golden_suite_generator/nanoBragg KMP_DUPLICATE_LIB_OK=TRUE python scripts/benchmarks/benchmark_detailed.py --sizes 4096 --device cpu --dtype float32 --profile --iterations 1 --keep-artifacts`; copy the emitted `reports/benchmarks/<ts>/` bundle into `reports/2026-01-vectorization-parity/phase_b/$STAMP/profile/`, capture `git rev-parse HEAD`, Python/torch versions, and record both benchmark correlation metrics (from `profile/benchmark_results.json`) and nb-compare metrics (`nb-compare --resample --threshold 0.999 --outdir .../nb_compare_full -- -default_F 100 -cell 100 100 100 90 90 90 -lambda 6.2 -distance 100 -detpixels 4096`) into `summary.md`. Halt and log the surprise if correlation ≥0.999 and |sum_ratio−1| ≤5e-3 on the fresh run.
  2. **Phase B2** — Execute `KMP_DUPLICATE_LIB_OK=TRUE NB_C_BIN=./golden_suite_generator/nanoBragg NB_RUN_PARALLEL=1 pytest -v tests/test_at_parallel_*.py -k 4096` and archive console output + exit status beside the Phase B1 bundle to confirm the regression reproduces under the authoritative parity selectors.
  3. **Phase B3** — Run targeted ROI parity checks (e.g., 1024² and 512² windows) via `nb-compare --roi ...` storing each result under `reports/2026-01-vectorization-parity/phase_b/$STAMP/nb_compare_roi_*` to determine whether the failure scales with detector size or specific regions.
  4. **Phase C Prep** — Once B1–B3 artifacts are in place, consolidate observed discrepancies (benchmark 0.721 vs nb-compare, sum_ratio ≫1, ROI scope) in `summary.md` and prepare trace tap candidates before handing off to Phase C callchain work.
- Attempts History:
