...
- Next Actions (2026-01-06 refresh — Phase C instrumentation greenlit):
  ...
  5. **Phase C3 — Trace diff & first divergence** — Produce `reports/.../first_divergence.md` summarising the earliest mismatched variable per pixel and update this entry's Attempts History with artifact links + hypotheses. Prioritize geometric variables (pix0_vector, basis vectors, scattering vector, Miller indices) since intensity scaling differs by design.
