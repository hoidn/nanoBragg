...
- Next Actions:
  1. 🛠️ Tap 5.1 (C mirror) — Add a `TRACE_C_TAP5_HKL` guard in `golden_suite_generator/nanoBragg.c` (HKL lookup block ≈ lines 3300-3355) emitting the same schema captured in Attempt #32/33 for pixels (2048,2048) and (0,0); capture logs + env metadata under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/c_taps/`.
  2. 📏 Tap 5.2 (HKL bounds parity) — Capture `TRACE_PY_HKL_BOUNDS` alongside `TRACE_C_HKL_BOUNDS` showing `[h_min,h_max]`, `[k_min,k_max]`, `[l_min,l_max]`; summarise findings in `tap5_hkl_bounds.md` and update this ledger + `plans/active/vectorization-parity-regression.md` before authorising remediation.
  3. 🧭 Hypothesis refresh — Fold Tap 5.1/5.2 evidence into `tap5_hypotheses.md`, re-rank H2/H3, and record the chosen remediation gate (Tap 6 vs Phase F) in both this ledger and `plans/active/vectorization-parity-regression.md`.
- Risks/Assumptions:
