...
0. **M2g Option B cache plumbing** — Adopt the batch-indexed pixel cache design from `reports/2025-10-cli-flags/phase_l/scaling_validation/20251208_option1_refresh/analysis.md` by threading `(slow_indices, fast_indices)` through the simulator, extending `Crystal.initialize_phi_cache/apply_phi_carryover/store_phi_final` to operate on batched pixels without `.clone()`/`.detach()`, and recording the architecture decision in `phi_carryover_diagnosis.md` with updated C-code citations.
