...
- Next Actions (2025-12-24 refresh):
  1. Capture a TC-D1 single-pixel trace (PyTorch + C) using `-trace_pixel 158 147` before touching simulator code; archive `py_trace.txt`, `c_trace.txt`, `diff.txt`, and `env.json` under `reports/2025-11-source-weights/phase_e/<STAMP>/trace/` to pinpoint the first divergence.
  2. Reconfirm regression coverage by running `KMP_DUPLICATE_LIB_OK=TRUE pytest tests/test_at_src_003.py -v` (with collect-only log) so TC-E1/E2/E3 stay green after recent merges; store outputs under the new Phase E timestamp.
