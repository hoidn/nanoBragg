...
0. **M2g.2c design brief** — Author `reports/2025-10-cli-flags/phase_l/scaling_validation/<timestamp>/optionB_batch_design.md` capturing batching granularity, tensor shapes, cache lifecycle, memory estimates, and validation cadence (pytest selectors + trace harness). Cite `specs/spec-a-core.md:205-233`, `docs/bugs/verified_c_bugs.md:166-204`, `nanoBragg.c:2797,3044-3095`, and reference `reports/.../20251208_option1_refresh/analysis.md` for prior analysis. No production edits.
1. **M2g.2d ROI prototype** — In the same timestamped directory, produce a disposable 4×4 ROI prototype (CPU float32) proving the Option B indexing works and retains gradients. Capture `prototype.md`, `metrics.json`, `commands.txt`, and a `gradcheck` snippet. Keep code outside production paths.
2. **M2g.3/M2g.4 wiring (after design sign-off)** — Once artifacts above are logged and reviewed, proceed with batch cache allocation + application per plan tasks M2g.3–M2g.4, then update parity tooling (M2g.5) and documentation (M2g.6).
3. **Phase M2h onward** — With cache wiring complete, run the validation bundle (CPU pytest selector, CUDA smoke if available, gradcheck) and refresh scaling traces (M2i). Follow with Phases N and O for nb-compare + supervisor rerun.
- Attempts History:
