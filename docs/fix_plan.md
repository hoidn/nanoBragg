...
- Next Actions (2025-12-24 status refresh):
  1. Phase D2: Draft `reports/2025-11-source-weights/phase_d/<STAMP>/design_notes.md` distilling the D1 divergence analysis into a single recommendation (spec amendment vs PyTorch implementation change). Include decision matrix, required code touchpoints, device/dtype risks, and acceptance metrics (correlation ≥0.999, |sum_ratio−1| ≤1e-3).
  2. Phase D3: Prepare the acceptance harness — define pytest selector(s) for the divergence + sourcefile scenario, record CLI command bundle with explicit `-oversample 1` and divergence flags, and capture `pytest --collect-only -q` evidence. Store under the same `<STAMP>` directory alongside `commands.txt` and `summary.md` scaffolding.
  3. Phase E1–E3: After D2/D3 sign-off, implement the chosen behaviour, extend regression coverage, and archive parity metrics (CPU mandatory, CUDA optional) before notifying `[VECTOR-GAPS-002]` / `[PERF-PYTORCH-004]` that profiling can resume.
- Attempts History:
