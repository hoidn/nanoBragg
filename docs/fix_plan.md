...
- Next Actions (2025-10-10 refresh — Phase C3 complete, ready for implementation):
  ...
  10. **Phase D5 — Apply fix + parity smoke** — Convert `rot_a/b/c` to m⁻¹ inside the simulator, remove NB_TRACE_SIM_F_LATT once parity is confirmed, then rerun ROI `nb-compare` (512² window) and refresh TRACE_PY logs under `reports/2026-01-vectorization-parity/phase_d/<STAMP>/phase_d_summary.md`. Require corr≥0.999 and |sum_ratio−1|≤5×10⁻³ with pytest collect still clean.
