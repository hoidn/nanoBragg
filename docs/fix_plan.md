...
- Next Actions (2025-10-08 Phase M1 complete):
  ...
  - **Phase M4 (OPEN)** — Phase M4a design memo captured at `reports/2025-10-cli-flags/phase_l/scaling_validation/20251008T223046Z/design_memo.md`; follow `plans/active/cli-noise-pix0/plan.md` Phase M4b–M4d checklist to remove the lingering double `/ steps` regression in `src/nanobrag_torch/simulator.py`, extend per-φ logging, re-run `pytest -v tests/test_cli_scaling_phi0.py` + `scripts/validation/compare_scaling_traces.py`, and bundle evidence under `reports/2025-10-cli-flags/phase_l/scaling_validation/fix_<timestamp>/` (summary/env/sha256/lattice_hypotheses update).
...
- Attempts History:
  * [2025-10-08] Attempt #192 (galph loop, Mode: Docs) — Result: ✅ **M4a COMPLETE / Fix reopened.** **No code changes.**
    ...
    Observations/Hypotheses:
      - PyTorch currently divides by `steps` twice: once when computing `normalized_intensity` and again when forming `physical_intensity`, producing the −14.6% I_before_scaling deficit observed in Phase M1.
      - Prior Attempt #188 reintroduced the double-division regression by adding `/ steps` at final scaling without removing the earlier normalization; Phase M4 remains open pending code correction.
    Next Actions:
      - Phase M4b: Update `src/nanobrag_torch/simulator.py` to align with the C contract (single division by `steps` alongside `r_e^2 * fluence`) while preserving vectorization/device neutrality and refreshing trace logs.
      - Phase M4c/M4d: Regenerate targeted pytest + compare_scaling_traces evidence and update `lattice_hypotheses.md` once parity is restored.
