...
- Priority: High
- Status: in_progress (Phase B3 validation-path decision outstanding)
...
- Next Actions (2026-01-01 refresh — Phase B scope now blocking parity triage):
 1. **Phase B3** — Draft a validation-path memo (`reports/2026-01-vectorization-parity/phase_b/$STAMP/validation_path.md`) comparing options for restoring 4096² parity coverage (un-skip AT-012 high-res test, add new parity case, or rely on benchmark/nb-compare only). Cite `tests/test_at_parallel_012.py:364` (current skip) and spec `specs/spec-a-parallel.md:90-99`; recommend a path and list acceptance criteria.
 2. **Phase B3.1** — Capture implementation requirements for the chosen path (golden data commands, ROI size, expected tolerances) and log blockers/approvals in `summary.md`; update plan/fix_plan attempts accordingly.
 3. **Phase B4** — After validation path is ratified, run ROI parity checks (e.g., 1024² and 512² windows) via `nb-compare --roi ...` storing each result under `reports/2026-01-vectorization-parity/phase_b/$STAMP/nb_compare_roi_*` to determine whether the failure scales with detector size or specific regions.
 4. **Phase C Prep** — Once B3/B4 artifacts are in place, consolidate observed discrepancies (benchmark 0.721 vs nb-compare, sum_ratio ≫1, ROI scope) in `summary.md` and prepare trace tap candidates before handing off to Phase C callchain work.
- Attempts History:
