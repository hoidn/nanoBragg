...
- Next Actions (2025-10-09 post-Attempt #23):
  1. **BLOCKER FIX REQUIRED**: Implement zero-weight source placeholder counting in PyTorch simulator steps calculation per nanoBragg.c:2700-2720. Location: `src/nanobrag_torch/simulator.py` where steps is computed. Must count ALL sources including zero-weight divergence placeholders generated by divergence grid logic.
  2. After steps fix, rerun TC-D1/TC-D3 parity (full-image, no trace pixel) and expect sum_ratio to drop from ~47-120× to ~23-60×. If still inflated beyond 2-4×, capture fresh trace to identify remaining scaling factors (likely per-source weight application or polarization issues).
  3. Execute the parity harness by rerunning both CLI surfaces (PyTorch: `KMP_DUPLICATE_LIB_OK=TRUE python -m nanobrag_torch ...`; C: `NB_RUN_PARALLEL=1 "$NB_C_BIN" ...`) and writing outputs to `reports/2025-11-source-weights/phase_e/<STAMP>/`. Capture correlation.txt, sum_ratio.txt, simulator diagnostics, env.json, commands.txt, and pytest logs; ensure corr ≥0.999 and |sum_ratio−1| ≤1e-3.
  4. Update documentation touchpoints (confirm `docs/architecture/pytorch_design.md` sources section reflects the override/steps semantics) and record the Attempt in this ledger with metrics + artifact paths.
  5. Notify `[VECTOR-GAPS-002]`/`[VECTOR-TRICUBIC-002]` once parity meets thresholds, flipping plan states (Phase E3 → done, Phase A2) and logging the unblock in galph_memory so profiling can resume.
- Attempts History:
