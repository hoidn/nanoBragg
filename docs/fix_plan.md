...
- Next Actions (2025-12-24 status refresh):
  1. Phase E2 fix: Patch `src/nanobrag_torch/__main__.py` to gate divergence auto-generation on `'sourcefile' not in config` (instead of `'sources' not in config`), ensuring the CLI never adds divergence sources when a sourcefile is supplied. Confirm TC-D2 still emits the documented UserWarning after the change.
  2. Phase E3 metrics: Re-run `NB_RUN_PARALLEL=1 NB_C_BIN=./golden_suite_generator/nanoBragg KMP_DUPLICATE_LIB_OK=TRUE pytest tests/test_cli_scaling.py::TestSourceWeightsDivergence -v` and capture a fresh `reports/2025-11-source-weights/phase_e/<STAMP>/` bundle (commands.txt, pytest.log, metrics.json, warning.log, env.json, summary.md) showing TC-D1/TC-D3 correlation ≥0.999 and |sum_ratio−1| ≤1e-3 once the guard fix lands.
  3. Phase E4/F1 follow-through: After parity succeeds, update `docs/architecture/pytorch_design.md` (Sources subsection) and, if needed, `specs/spec-a-core.md` to document the precedence rule; then notify `[VECTOR-GAPS-002]` and `[PERF-PYTORCH-004]` that profiling can resume and archive the stabilized artifacts.
- Attempts History:
