...
**Last Updated:** 2026-01-10 (galph loop — VECTOR-PARITY Phase E Tap 4 comparison prep)
**Active Focus:**
- Leverage Attempt #28 Tap 4 results to capture Tap 5 (pre-normalisation intensity) for PyTorch and C, then choose the next parity probe (Tap 6 water background or Phase F remediation).
...
- Observations/Hypotheses:
  ...
  - Attempts #26–#27 reveal a parity gap: PyTorch applies `default_F=100` at the centre pixel even for in-bounds HKL lookups, while C returns 0.0 from the empty Fhkl array. Need to confirm whether the fallback should trigger in this configuration before proceeding to other taps.
- Next Actions:
  1. 🛠️ Extend `scripts/debug_pixel_trace.py` to expose Tap 5 (`intensity_pre_norm`) metrics (accumulated intensity, steps, last ω) with `--taps intensity` + `--json` support; rerun for pixels (0,0) + (2048,2048) with oversample=2 and archive outputs under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/py_taps/` alongside `pre_norm_summary.md`.
  2. 🔧 Mirror Tap 5 on the C side by instrumenting `golden_suite_generator/nanoBragg` (guarded by `#ifdef TRACE_C_TAP5`) to emit raw/normalised intensity prior to scaling for the same pixels; capture logs in `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/c_taps/` and verify clean rebuild (`pytest --collect-only -q`).
  3. 🧭 Compare PyTorch vs C Tap 5 metrics in `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/comparison/intensity_pre_norm.md`, decide whether Tap 6 (water background) is required or Phase F remediation can begin, and update this ledger + `plans/active/vectorization-parity-regression.md` accordingly.
- Risks/Assumptions:
