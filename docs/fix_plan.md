...
- Next Actions:
  1. 🧭 Tap 5.1 (PyTorch HKL audit) — Extend `scripts/debug_pixel_trace.py --taps hkl_subpixel` to log fractional HKL, rounded `(h0,k0,l0)`, `F_cell`, and `out_of_bounds` for pixel (2048,2048) at oversample=2. Archive JSON + summary under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/py_taps/`, citing `specs/spec-a-core.md:232-240` for the nearest-neighbour fallback contract.
  2. 🛠️ Tap 5.1 (C mirror) — Add a `TRACE_C_TAP5_HKL` guard in `golden_suite_generator/nanoBragg.c` (HKL lookup block ≈ lines 3300-3355) emitting the same schema for pixels (2048,2048) and (0,0); capture logs + env metadata under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/c_taps/`.
  3. 📏 Tap 5.2 (HKL bounds parity) — Capture `TRACE_PY_HKL_BOUNDS` alongside `TRACE_C_HKL_BOUNDS` showing `[h_min,h_max]`, `[k_min,k_max]`, `[l_min,l_max]`; summarise findings in `tap5_hkl_bounds.md` and update this ledger + `plans/active/vectorization-parity-regression.md` before authorising remediation.
- Risks/Assumptions:
