...
- Next Actions (2025-12-24 refresh):
  1. Capture SOURCE-WEIGHT-001 Phase E parity bundle per `plans/active/source-weight-normalization.md` (rerun PyTorch + C CLI commands for TC-D1/TC-D3 with corr ≥0.999, |sum_ratio−1| ≤1e-3). Store artifacts under `reports/2025-11-source-weights/phase_e/<STAMP>/` and log the Attempt in `[SOURCE-WEIGHT-001]`.
  2. After parity metrics land, flip `plans/active/vectorization-gap-audit.md` Phase B1 to READY, update this entry and galph_memory with the unblock timestamp, and refresh `[VECTOR-GAPS-002]` Next Actions accordingly.
  3. Run Phase B regression refresh commands (tricubic/absorption pytest suites + microbenchmarks) and archive outputs under `reports/2026-01-vectorization-refresh/phase_b/<STAMP>/` with before/after comparisons to the 2025-10 baselines.
  4. Resume VECTOR-GAPS-002 Phase B via plan Phase C (profiler capture, hot loop mapping, backlog publication). Keep PERF-PYTORCH-004 coordination in sync when profiling restarts.
...
- Owner/Date: galph/2025-11-17 (updated 2025-12-24)
...
- Next Actions (refreshed 2025-12-24 after ledger sync + Phase F design packet):
  ...
  - Phase E2 ✅ Ledger + dependency propagation complete this loop (see galph_memory 2025-12-24 entry, `plans/active/vectorization*.md` updates). No further action.
  - Phase F1–F3 ✅ Design packet archived at `reports/2025-11-source-weights/phase_f/20251009T203823Z/` (test_plan.md, commands.txt, collect.log).
  1. Phase G1 — Update `tests/test_cli_scaling.py::TestSourceWeights*` per the design packet (weights ignored, CLI lambda override, expected C divergence marked xfail). Reference `plans/active/source-weight-normalization.md` Phase G1 for task checklist.
  2. Phase G2 — Capture the evidence bundle: `KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests/test_cli_scaling.py::TestSourceWeights tests/test_cli_scaling.py::TestSourceWeightsDivergence` plus the TC-D1/TC-D3 CLI commands, archiving under `reports/2025-11-source-weights/phase_g/<STAMP>/` with metrics labelled "expected divergence".
  3. Phase G3 — Log the new Attempt with artifacts/metrics and update dependent plans once tests are green; leave notes for Phase H follow-up (docs + downstream notifications).
  4. Phase H1–H3 — After Phase G evidence is accepted, sync permanent docs and dependent plans, then prepare closure summary + archive.
- Attempts History:
