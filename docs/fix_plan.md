...
- Attempts History:
  ...
  * [2025-09-30] Attempt #7 — Result: failed (approximation only). Commit `fcbb93a` updated the polarization paths to reuse the primary source vector, but secondary sources remain unpolarized so physics still diverges from nanoBragg.c.
    ...
    Observations/Hypotheses: The primary-source shortcut at `src/nanobrag_torch/simulator.py:775-823, 894-951` still violates spec — each source needs its own Kahn factor before reduction. Speedup measurements are valid but parity remains broken.
    Next Actions: Reopen P3.0b with a per-source polarization refactor, then proceed to P3.0c normalization verification once parity metrics match the C trace.
  * [2025-10-04] Attempt #8 — Result: stalled. Supervisor review found the fallback path for `source_wavelengths` still crashes when callers supply directions without explicit wavelengths (`src/nanobrag_torch/simulator.py:431` still calls `.to(...)` on `None`). Multi-source polarization still needs per-source weighting—current implementation reuses only the primary source vector (lines ~822, 952), so secondary sources remain unpolarized—and normalization continues to average over sources via `steps` (lines 687-695), keeping P3.0c blocked. ROI mask/omega tensors still rebuild every run (lines 543-615, 905-918), leaving Phase P3.4 perf targets unmet.
    Metrics: n/a (analysis only).
    Artifacts: n/a.
    Observations/Hypotheses: P3.0 must materialize default wavelengths/weights before the `.to()` call and P3.0b should either pass per-source directions into the polarization factor or move the calculation alongside the intensity sum; P3.0c needs the normalization to divide only by phi/mosaic/oversample once sources have been accumulated.
    Next Actions: 1) Patch `Simulator.__init__` per P3.0 guidance to seed missing wavelengths/weights from the primary beam tensor on the correct device/dtype; 2) Revisit polarization so each source contributes its own Kahn factor before summation; 3) Finish P3.0c and resume P3.4 ROI caching after physics parity is restored.
