...
- Next Actions:
  1. 🔍 Execute Tap 2/3 (omega asymmetry quantification): Run `KMP_DUPLICATE_LIB_OK=TRUE NB_TRACE_EDGE_PIXEL="0,0" python scripts/debug_pixel_trace.py --pixel 0 0 --oversample 2 --out-dir reports/2026-01-vectorization-parity/phase_e0/<STAMP>/py_taps/`; compute `relative_asymmetry` vs center pixel (2048,2048) and archive `omega_analysis.md`.
  2. 📖 Generate C trace for pixel (0,0): Instrument `golden_suite_generator/nanoBragg` oversample loop per `trace/tap_points.md`; capture all four subpixel ω values into `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/c_taps/` with command log.
  3. 🔬 Compare C↔PyTorch omega handling: Produce `omega_comparison.md` summarising whether C averages ω; if evidence inconclusive, halt and expand taps before touching production code.
  4. 🧪 If C averages ω, implement averaging in `simulator.py` oversample branch (delegate), then rerun 4096² nb-compare (full-frame + ROI) and capture metrics under `reports/2026-01-vectorization-parity/phase_f/<STAMP>/`; if parity still <0.999, escalate with new failure summary.
  5. 📝 Update docs/fix_plan.md Attempts and this plan’s Phase F table with the new evidence, then notify `[VECTOR-TRICUBIC-002]` that profiling gate can lift once corr≥0.999 and |sum_ratio−1|≤5×10⁻³ are documented.
- Risks/Assumptions:
