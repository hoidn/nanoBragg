...
- Next Actions (2025-12-24 refresh):
  1. Reconfirm regression coverage by running `KMP_DUPLICATE_LIB_OK=TRUE pytest tests/test_at_src_003.py -v` (with collect-only log) so TC-E1/E2/E3 stay green after recent merges; store outputs under the new Phase E timestamp.
  2. Execute the parity harness by rerunning both CLI surfaces (PyTorch: `KMP_DUPLICATE_LIB_OK=TRUE python -m nanobrag_torch ...`; C: `NB_RUN_PARALLEL=1 "$NB_C_BIN" ...`) and writing outputs to `reports/2025-11-source-weights/phase_e/<STAMP>/`. Capture correlation.txt, sum_ratio.txt, simulator diagnostics, env.json, commands.txt, and pytest logs; ensure corr ≥0.999 and |sum_ratio−1| ≤1e-3.
  3. Update documentation touchpoints (confirm `docs/architecture/pytorch_design.md` sources section reflects the override/steps semantics) and record the Attempt in this ledger with metrics + artifact paths.
  4. Notify `[VECTOR-GAPS-002]`/`[VECTOR-TRICUBIC-002]` once parity meets thresholds, flipping plan states (Phase E3 → done, Phase A2) and logging the unblock in galph_memory so profiling can resume.
- Attempts History:
