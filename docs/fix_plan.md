...
1. **M2d cross-pixel probe** — Run `reports/2025-10-cli-flags/phase_l/scaling_audit/trace_harness.py --roi 684 686 1039 1040 --phi-mode c-parity` (CPU float64) alongside the C trace to prove φ=0 still recomputes per pixel. Archive outputs under `reports/.../carryover_probe/<timestamp>/` and append deltas to `lattice_hypotheses.md`.
2. **M2f Option 1 design detail** — Extend `phi_carryover_diagnosis.md` with the pixel-indexed cache data flow: tensor shapes `(S,F,N_mos,3)`, cache reset rules, simulator entry points, and memory budget. This becomes the implementation spec for Ralph before any code edits.
3. **M2g–M2h implementation & validation** — Rework the carryover path using the Option 1 design (no `.detach()`, vectorised scatter/index ops, cache stored on caller device/dtype) and run the targeted pytest/gradcheck bundle (`pytest tests/test_cli_scaling_parity.py::TestScalingParity::test_I_before_scaling_matches_c`, CUDA variant when available, minimal gradcheck). Store logs under `reports/.../carryover_cache_validation/<timestamp>/` with env/commands metadata.
4. **M3+ downstream checks** — Once the cache passes validations, rerun `trace_harness.py` + `scripts/validation/compare_scaling_traces.py` to drive `metrics.json` to `first_divergence=None`, update the scaling parity regression test with the passing log, then proceed to nb-compare (Phase N) and supervisor rerun (Phase O).
- Attempts History:
