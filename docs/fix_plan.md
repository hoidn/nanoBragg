...
- Next Actions:
  ...
  2. Execute Phase D2 docstring refresh for `src/nanobrag_torch/utils/c_random.py` (module + `mosaic_rotation_umat()` RNG notes) and capture artifacts under the Phase D docs integration bundle.
  3. Update `arch.md` ADR-05 with the pointer-side-effect implementation note (Phase D3) per `docs_updates.md` §1.2.
  4. Integrate the determinism workflow (env guards + selectors) into `docs/development/testing_strategy.md` §2.6 (Phase D4) using `testing_strategy_notes.md` as source.
  5. After D2–D4 merge, run Phase E validation (determinism pytest selectors with env guards) and record closure summary before marking `[DETERMINISM-001]` done / archiving the plan.
- Attempts History:
