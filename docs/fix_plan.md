...
1. **M2e scaling-parity test harness** — Keep `tests/test_cli_scaling_parity.py::TestScalingParity::test_I_before_scaling_matches_c` red but reproducible; capture the current failure output (CPU float64, Δ≈0.209%) under a new timestamp and attach it to `lattice_hypotheses.md` before implementation.
2. **M2g–M2h implementation & validation** — Implement the Option 1 pixel-indexed cache per `reports/2025-10-cli-flags/phase_l/scaling_validation/20251008T100653Z/analysis.md` (no `.detach()`, device/dtype neutral), then run the targeted pytest/gradcheck bundle (`pytest tests/test_cli_scaling_parity.py::TestScalingParity::test_I_before_scaling_matches_c`, CUDA variant when available) and archive logs under `.../carryover_cache_validation/<timestamp>/` with env metadata.
3. **M2i + M3 reruns** — After the cache lands, rerun the cross-pixel harness (`trace_harness.py --roi 684 686 1039 1040 --phi-mode c-parity`, CPU float64) and the single-pixel comparison to drive `metrics.json` to `first_divergence=None`, updating `scaling_validation_summary.md`.
4. **M4+ downstream checks** — Once VG‑2 is green, refresh documentation (M4) and proceed to nb-compare (Phase N) plus the supervisor parity rerun (Phase O).
- Attempts History:
