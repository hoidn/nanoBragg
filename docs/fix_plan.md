...
- Next Actions (2025-10-10 refresh — Phase C3 complete, ready for implementation):
  ...
  6. **Phase D1 — Fix H1 (scattering_vec unit conversion)** — Search `simulator.py`/`utils/physics.py` for `q = (d - i) / lambda`; verify λ units (should be Å, not m) and ensure output is in Å⁻¹. Rerun Phase C2 traces to validate scattering_vec matches C-code to ≤10⁻⁶ relative.
  7. **Phase D2 — Fix H2 (fluence calculation)** — Audit `config.py` (BeamConfig) or simulator scaling for unit consistency; ensure all distances in meters, areas in m². Check fluence matches C-code to ≤10⁻³ relative.
  8. **Phase D3 — Fix H3 (F_latt normalization)** — Compare PyTorch `utils/physics.py::sincg` with C-code implementation (nanoBragg.c ~lines 15000-16000); verify crystal size (Na×Nb×Nc) normalization. F_latt should match C-code for background pixels to ≤10⁻² relative.
  9. **Phase E — Full validation** — After all fixes, rerun nb-compare full-frame (expect corr ≥ 0.999), ROI comparison (expect |sum_ratio−1| ≤ 5×10⁻³), and `test_at_parallel_012` (expect pass without xfail).
- Risks/Assumptions:
...
- Priority: High
- Status: in_progress (Phase B regression refresh complete)
