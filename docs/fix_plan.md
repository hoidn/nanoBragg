...
- Priority: High
- Status: in_progress (Phase B2 parity selectors outstanding)
...
- Next Actions (2026-01-01 refresh — Phase B scope now blocking parity triage):
 1. **Phase B2** — Execute `KMP_DUPLICATE_LIB_OK=TRUE NB_C_BIN=./golden_suite_generator/nanoBragg NB_RUN_PARALLEL=1 pytest -v tests/test_at_parallel_*.py -k 4096` and archive console output + exit status beside the Phase B1 bundle to confirm the regression reproduces under the authoritative parity selectors.
 2. **Phase B2.5** — Compare float binaries produced by benchmark_detailed.py vs nb-compare (if available) to explain the 0.721 vs 0.063 correlation delta; record findings in `summary.md`.
 ...
 4. **Phase C Prep** — Once B2/B3 artifacts are in place, consolidate observed discrepancies (benchmark 0.721 vs nb-compare, sum_ratio ≫1, ROI scope) in `summary.md` and prepare trace tap candidates before handing off to Phase C callchain work.
- Attempts History:
