...
- Next Actions:
  1. 🧮 Compile `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/comparison/intensity_pre_norm.md` merging Tap 5 edge/centre metrics from Attempts #29–#30 (PyTorch vs C). Quantify each factor (I_before_scaling, steps, ω, capture, polar) and highlight which terms cancel vs diverge. Summarise ratios + raw values so the 4× delta is explicit.
  2. 🔍 Draft `tap5_hypotheses.md` (same bundle) ranking candidate causes for the residual (e.g., per-subpixel accumulation order, lattice factor scaling, implicit background terms) and propose the next evidence hook (Tap 6 water background vs deeper Tap 5 per-subpixel audit). Update this plan and `plans/active/vectorization-parity-regression.md` with the selected path.
- Risks/Assumptions:
