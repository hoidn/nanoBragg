...
- Next Actions:
  1. ✅ Draft Phase E0 callchain brief — COMPLETE (Attempt #22, `reports/2026-01-vectorization-parity/phase_e0/20251010T092845Z/callchain_brief.md`).
  2. ✅ Execute callchain tracing for edge pixel — COMPLETE (Attempt #22, deliverables: callchain/static.md, trace/tap_points.md, summary.md, env/trace_env.json).
  3. 🔍 Execute Tap 2/3 (omega asymmetry quantification): Generate PyTorch trace with `NB_TRACE_EDGE_PIXEL="0,0"` for pixel (0,0) with oversample=2; capture `omega_all` tensor; compute `relative_asymmetry = |omega_last - omega_mean| / omega_mean`; compare to center pixel (2048,2048) baseline.
  4. 📖 Generate C trace for pixel (0,0): Add printf instrumentation to C code oversample loop; capture omega values for all 4 subpixels; determine if C averages omega or uses last-value.
  5. 🔬 Compare C↔PyTorch omega handling: If C averages but PyTorch uses last-value, implement averaging in PyTorch `simulator.py:1027-1030`; rerun full-frame benchmark; expect correlation to improve from 0.721 → >0.99.
  6. 📌 Update `plans/active/vectorization-parity-regression.md` Phase E0d with tap outcomes and mark E0b/E0c [D]one; only advance to Phase E1 full-frame rerun after omega fix validated.
- Risks/Assumptions:
