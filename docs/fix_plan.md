...
- Next Actions:
  1. Re-execute Phase A per `plans/active/determinism.md`: rerun AT-PARALLEL-013/024 with fresh timestamped artifacts now that dtype crashes no longer short-circuit the suite.
  2. Summarise the new failure signatures (expected TorchDynamo CUDA device errors) in `phase_a/<STAMP>/summary.md` and update this ledger as Attempt #2, noting any seed-related behaviours unlocked by the rerun.
  3. Classify remaining failures: seed-related vs infrastructure (Dynamo) issues
