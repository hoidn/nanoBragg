...
- Attempts History:
  ...
  * [2025-10-10] Attempt #13 — Result: ⚠️ partial (Phase D3 trace helper). Imported production `sincg` into `scripts/debug_pixel_trace.py`, captured `f_latt_parity.md` with rel_err≤5e-15, and documented outcomes in `reports/2026-01-vectorization-parity/phase_d/20251010T071935Z/PHASE_D3_SUMMARY.md`. ROI `nb-compare` now fails (corr=-0.001, sum_ratio=12.54) because `Simulator.run()` still emits ~32× lower intensity; see accompanying `blockers.md` for reproduction and next steps. Next: instrument simulator F_latt pipeline (Phase D4) before re-running parity smoke.
- Observations/Hypotheses:
  ...
  - Attempt #13 exposes a simulator-specific F_latt regression: trace helper parity is correct, but `Simulator.run()` returns ~32× less intensity (corr=-0.001 ROI) — see `reports/2026-01-vectorization-parity/phase_d/20251010T071935Z/blockers.md` (Option B recommends spinning a dedicated simulator remediation phase).
- Next Actions (2025-10-10 refresh — Phase C3 complete, ready for implementation):
  ...
  8. ✅ **Phase D3 — Trace helper F_latt normalization** — Attempt #13 (`reports/2026-01-vectorization-parity/phase_d/20251010T071935Z/PHASE_D3_SUMMARY.md`) imports production `sincg` into the trace helper and matches TRACE_C within 5e-15, but ROI parity now fails (corr=-0.001, sum_ratio=12.5×) because the simulator still outputs ~32× less intensity. Treat this as partial and escalate to Phase D4.
  9. **Phase D4 — Diagnose simulator F_latt regression (32× intensity gap)** — Instrument `Simulator._compute_physics_for_position()` to log `h/k/l`, `F_latt_a/b/c`, `F_cell`, and per-loop accumulators for pixel (1792,2048); diff against `scripts/debug_pixel_trace.py` and the C trace; summarise findings under `reports/2026-01-vectorization-parity/phase_d/<STAMP>/simulator_f_latt.md`. Exit when simulator intensity agrees with the trace within ≤1e-3 and ROI nb-compare hits corr≥0.999 with |sum_ratio−1|≤5×10⁻³.
  10. **Phase D5 — Consolidated parity smoke** — After D4 fix, rerun ROI `nb-compare` (512² window) and refresh TRACE_PY logs under `reports/2026-01-vectorization-parity/phase_d/<STAMP>/`, updating `phase_d_summary.md` with corr/sum_ratio metrics. Ensure pytest collect remains green.
  11. **Phase E — Full validation sweep** — Execute full-frame benchmark + high-res pytest per plan Phase E once D1–D5 artifacts are green; update ledgers/guardrails before closing the item.
- Risks/Assumptions:
