...
**Active Focus:**
- Convert Attempt #26 (PyTorch) and Attempt #27 (C) Tap 4 evidence into a consolidated comparison and decide the next parity hypothesis (Tap 5 vs Tap 6).
...
- Next Actions:
  1. 📝 Draft `f_cell_comparison.md` under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/` consolidating Attempt #26 (PyTorch) and Attempt #27 (C) Tap 4 metrics, highlighting the centre-pixel divergence and expected spec behaviour.
  2. 🔍 Audit PyTorch’s structure-factor fallback path (trace helper vs `models/crystal.py`) to determine why the centre pixel reports `default_F=100`; capture findings in `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/default_f_audit.md` with code pointers and spec citations.
  3. 🧭 Based on the audit outcome, update `plans/active/vectorization-parity-regression.md` and this ledger with the selected next tap (Tap 5 pre-normalisation intensity or Tap 6 water background) and refresh `input.md` Do Now accordingly.
- Risks/Assumptions:
