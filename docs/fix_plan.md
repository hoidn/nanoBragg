...
**Last Updated:** 2026-01-10 (galph loop — VECTOR-PARITY Phase E Tap 5 C instrumentation prep)
**Active Focus:**
- Use Attempt #29 PyTorch Tap 5 evidence to instrument the C binary, compare intensity pre-norm metrics, and decide whether to escalate to Tap 6 water background or move into Phase F remediation.
...
- Next Actions:
  1. 🔧 Instrument `golden_suite_generator/nanoBragg` with a `TRACE_C_TAP5` guard (compile-time macro or runtime `getenv`) around the intensity accumulation loop (approx. lines 3770–3815) to log `I_before_scaling`, step counts, and the last ω/capture/polar values for pixels (0,0) and (2048,2048) at oversample=2. Store outputs under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/c_taps/`, include `commands.txt` + `env/trace_env.json`, and verify a clean rebuild with `KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q`.
  2. 🧭 Compare Tap 5 metrics (PyTorch vs C) in `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/comparison/intensity_pre_norm.md`, conclude whether the defect lives pre-normalisation or requires Tap 6 (water background) follow-up, and update this ledger plus `plans/active/vectorization-parity-regression.md` with the decision.
- Risks/Assumptions:
