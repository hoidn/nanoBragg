...
- Next Actions (2025-12-24 status refresh):
  1. Phase D3: Using the D2 design decision (`reports/2025-11-source-weights/phase_d/20251009T103212Z/design_notes.md`), stage the acceptance harness: create `commands.txt` with paired C/Py commands (explicit `-oversample 1`, divergence flags, warning expectation), capture `pytest tests/test_cli_scaling.py::TestSourceWeightsDivergence -v` (plus `--collect-only -q` log), and scaffold `summary.md` enumerating required metrics (correlation ≥0.999, |sum_ratio−1| ≤1e-3, warning captured).
  2. Phase E1–E3: After D3 assets land, implement Option B (spec clarification + validation guard), extend regression coverage, and archive parity metrics (CPU mandatory, CUDA optional) before notifying `[VECTOR-GAPS-002]` / `[PERF-PYTORCH-004]` that profiling can resume.
- Attempts History:
