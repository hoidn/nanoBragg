#!/usr/bin/env python3
"""
Refinement Demo (Diffuse): Small Crystal with Broad Spots

This demo uses a smaller crystal (N_cells=3,3,3) and higher mosaic spread
to produce broader, more diffuse diffraction spots compared to the other
refinement demos. Combined with a larger detector (256x256), this gives
a visual appearance similar to demo_rotation.py outputs.

This demo is useful for:
- Demonstrating refinement with broader, more realistic spot profiles
- Testing optimization with higher mosaicity
- Comparing convergence behavior vs sharp-spot demos

Note: Due to the broader spots and higher mosaic spread, optimization
behavior differs from the sharp-spot demos.

Reference: plans/active/refinement-demo.md

Usage:
    python scripts/refinement_demo_diffuse.py [options]

Options:
    --iterations N    Number of optimization iterations (default: 300)
    --fallback        Use single-parameter (mosaic-only) mode (recommended)
    --plot            Generate static visualization (PNG)
    --gif             Generate animated GIF showing refinement progression
    --frame-interval  Save frame every N iterations for GIF (default: 10)
    --fps N           Frames per second for GIF (default: 6)

Examples:
    # Recommended: mosaic-only refinement with visualization
    python scripts/refinement_demo_diffuse.py --fallback --plot

    # Full joint refinement (may converge slowly)
    python scripts/refinement_demo_diffuse.py --plot --iterations 400

    # With GIF animation
    python scripts/refinement_demo_diffuse.py --fallback --plot --gif
"""
...
# ============================================================================
# CONFIGURATION - Single Unit Cell (Diffuse Spots)
# ============================================================================

# Ground truth parameters to recover
# Using higher mosaic spread since broad spots make small spreads less visible
TRUE_MOSAIC_SPREAD = 2.0  # degrees (higher than sharp-spot demos)
TRUE_MISSET = (3.0, 2.0, -1.5)  # degrees (smaller than sharp demos - harder to detect)

# Fixed parameters (not refined)
# Smaller N_cells = broader spots (but not single unit cell)
FIXED_PARAMS = dict(
    cell_a=100.0,
    cell_b=100.0,
    cell_c=100.0,
    ...
    cell_beta=90.0,
    cell_gamma=90.0,
    N_cells=(3, 3, 3),  # Smaller crystal = broader, more diffuse spots
    default_F=100.0,
    mosaic_domains=20,  # More domains for smooth appearance
    mosaic_seed=42,  # CRITICAL: deterministic sampling for gradients
)

# Detector configuration - larger for diffuse visual appearance
DETECTOR_CONFIG = DetectorConfig(
    fpixels=256,  # Fast axis pixels
    spixels=256,  # Slow axis pixels
    pixel_size_mm=0.1,
    distance_mm=100.0,
)
...
def generate_experimental_data(
    ...
    print("Generated experimental data (DIFFUSE - single unit cell):")
    ...
    print(f"  N_cells: {FIXED_PARAMS['N_cells']} (single unit cell = broad spots)")
...
def run_refinement(
    ...
    Note: Diffuse spots have shallower gradients, requiring:
    - Lower learning rates
    - More iterations
    - Patience with convergence
    """
    # Start closer to truth for diffuse case (harder optimization)
    init_mosaic = torch.tensor(0.5, dtype=dtype, requires_grad=True)
    ...
    if use_fallback:
        ...
        # Lower LR for diffuse patterns
        optimizer = torch.optim.Adam(params, lr=0.02)
    else:
        # Start with small offset from zero
        init_misset_x = torch.tensor(0.5, dtype=dtype, requires_grad=True)
        init_misset_y = torch.tensor(0.5, dtype=dtype, requires_grad=True)
        init_misset_z = torch.tensor(-0.5, dtype=dtype, requires_grad=True)
        # Very conservative learning rates for diffuse patterns
        optimizer = torch.optim.Adam([
            {"params": [init_mosaic], "lr": 0.015},
            {"params": [init_misset_x, init_misset_y, init_misset_z], "lr": 0.03},
        ])
    ...
    # More patient scheduler for diffuse patterns
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.7, patience=25, min_lr=1e-4
    )

    print(f"\n{'='*60}")
    print(f"Starting refinement: {n_iterations} iterations")
    print(f"Note: Diffuse patterns converge more slowly than sharp spots")
    print(f"{'='*60}")
    ...
    for iteration in range(n_iterations):
        ...
        with torch.no_grad():
            init_mosaic.clamp_(min=0.01, max=10.0)  # Higher max for diffuse
...
def print_results(history: dict, use_fallback: bool = False) -> dict:
    ...
    print(f"\n{'='*60}")
    print("REFINEMENT RESULTS (DIFFUSE SPOTS)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("SUCCESS CRITERIA (relaxed for diffuse patterns)")
    print(f"{'='*60}")

    success = True

    # Relaxed criteria for diffuse patterns
    if reduction >= 5.0:
        print(f"  Loss reduction >= 5x: {reduction:.1f}x")
    else:
        print(f"  Loss reduction >= 5x: {reduction:.1f}x (below threshold)")
        success = False

    # Relaxed mosaic tolerance for diffuse
    if mosaic_error <= 0.3:
        print(f"  Mosaic spread error <= 0.3: {mosaic_error:.3f}")
    else:
        print(f"  Mosaic spread error <= 0.3: {mosaic_error:.3f} (above threshold)")
        success = False

    if not use_fallback:
        max_misset_err = max(misset_errors)
        # Relaxed misset tolerance for diffuse
        if max_misset_err <= 1.5:
            print(f"  Max misset error <= 1.5: {max_misset_err:.2f}")
        else:
            print(f"  Max misset error <= 1.5: {max_misset_err:.2f} (above threshold)")
            success = False
    ...
    print(f"\n{'='*60}")
    if success:
        print("REFINEMENT CONVERGED WITHIN TOLERANCES")
    else:
        print("REFINEMENT DID NOT FULLY CONVERGE")
        print("(This is expected for diffuse patterns - try --fallback mode)")
    print(f"{'='*60}")
...
def plot_results(
    ...
    use_fallback: bool = False,
    output_path: str = "refinement_demo_diffuse.png",
) -> None:
    ...
    fig.suptitle("Diffuse Pattern Refinement (Single Unit Cell)", fontsize=14, fontweight="bold")
    ...
    im2 = axes[0, 2].imshow(diff, cmap="RdBu_r", origin="lower", vmin=-vmax_diff, vmax=vmax_diff)
    ...
    axes[1, 1].set_ylabel("Mosaic Spread (deg)")
    ...
    if not use_fallback:
        ...
        axes[1, 2].set_ylabel("Misset Angles (deg)")
...
def create_refinement_gif(
    frames: list,
    experimental_image: torch.Tensor,
    output_path: str = "refinement_animation_diffuse.gif",
    fps: int = 6,
    use_fallback: bool = False,
) -> None:
    ...
    fig.suptitle("Diffuse Pattern Refinement Progress", fontsize=14, fontweight="bold")
    ...
    plt.colorbar(im_diff, ax=axes[2], fraction=0.046, label="Delta Intensity")
    ...
    def update(frame_idx):
        ...
        if use_fallback:
            text = (
                ...
                f"Mosaic: {frame['mosaic']:.3f} deg (true: {TRUE_MOSAIC_SPREAD} deg)"
            )
        else:
            text = (
                f"Iteration: {frame['iteration']}\n"
                f"Loss: {frame['loss']:.3e}\n"
                f"Mosaic: {frame['mosaic']:.3f} deg (true: {TRUE_MOSAIC_SPREAD} deg)\n"
                f"Misset X: {frame['misset'][0]:+.2f} deg (true: {true_misset[0]:+.1f} deg)\n"
                f"Misset Y: {frame['misset'][1]:+.2f} deg (true: {true_misset[1]:+.1f} deg)\n"
                f"Misset Z: {frame['misset'][2]:+.2f} deg (true: {true_misset[2]:+.1f} deg)"
            )
...
def main():
    """Main entry point for diffuse refinement demo."""
    parser = argparse.ArgumentParser(
        description="Refinement Demo (Diffuse): Single unit cell with broad spots"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Use single-parameter (mosaic-only) mode (recommended for diffuse)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Save a frame every N iterations for GIF (default: 10)",
    )
    parser.add_argument(
        "--fps", type=int, default=6,
        help="Frames per second for GIF animation (default: 6)",
    )
    ...
    print("=" * 60)
    print("NANOBRAG PYTORCH REFINEMENT DEMO (DIFFUSE)")
    print("Single Unit Cell - Broad Diffraction Spots")
    print("=" * 60)
    print(f"\nDevice: {device}")
    print(f"Dtype: {dtype}")
    print(f"Mode: {'fallback (mosaic-only)' if args.fallback else 'full (mosaic + misset)'}")
    print(f"Iterations: {args.iterations}")
    print(f"\nNote: N_cells=(1,1,1) produces broad, diffuse spots")
    print("      Optimization is slower than with sharp spots")
    ...
    if args.plot or args.gif:
        ...
        if args.plot:
            output_path = OUTPUT_DIR / "refinement_demo_diffuse.png"
            plot_results(
        ...
        if args.gif and frames:
            gif_path = OUTPUT_DIR / "refinement_animation_diffuse.gif"
