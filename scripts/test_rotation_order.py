...
def test_detector_rotation_simple():
    """Simple test to prevent pytest collection errors."""
    ...
    print("✅ Detector with tilted configuration created successfully")


if __name__ == "__main__":
    # Only run main script when executed directly
    from nanobrag_torch.config import DetectorConfig, DetectorConvention, DetectorPivot
    from nanobrag_torch.models.detector import Detector
    ...
    # Convert degrees to radians and make tensors
    rotx_rad = torch.tensor(np.radians(config.detector_rotx_deg), dtype=torch.float64)
    roty_rad = torch.tensor(np.radians(config.detector_roty_deg), dtype=torch.float64)
    rotz_rad = torch.tensor(np.radians(config.detector_rotz_deg), dtype=torch.float64)
    twotheta_rad = torch.tensor(np.radians(config.detector_twotheta_deg), dtype=torch.float64)
    ...
    fdet_final = rotate_around_axis(fdet, twotheta_axis, twotheta_rad)
    sdet_final = rotate_around_axis(sdet, twotheta_axis, twotheta_rad)
    odet_final = rotate_around_axis(odet, twotheta_axis, twotheta_rad)
