"""
CLI Scaling Parity Test for CLI-FLAGS-003 Phase M2

Tests that PyTorch implementation matches C reference values for the scaling chain
when using c-parity mode (which emulates the C code's φ=0 vector carryover bug
documented in C-PARITY-001).

Evidence base: reports/2025-10-cli-flags/phase_l/scaling_validation/20251008T075949Z/
Plan reference: plans/active/cli-noise-pix0/plan.md Phase M2
Supervisor memo: input.md:1-8 (Do Now for Phase M2)
"""
import os
import sys
...
class TestScalingParity:
    """Validate that I_before_scaling matches C trace when using c-parity mode."""
    ...
    def test_I_before_scaling_matches_c(self):
        ...
        # Validate I_before_scaling
        i_before_rel_error = abs(i_before_py - expected_i_before) / abs(expected_i_before)
        assert i_before_rel_error <= tolerance, \
            f"I_before_scaling relative error {i_before_rel_error:.6g} > {tolerance}. " \
            f"Expected {expected_i_before:.10f}, got {i_before_py:.10f}. " \
            f"Absolute delta: {i_before_py - expected_i_before:.10g}."
