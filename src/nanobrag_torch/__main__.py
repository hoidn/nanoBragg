...
def save_smv_image(image: torch.Tensor, filename: str, config: DetectorConfig,
    ...
    header_lines = [
        ...
        f"PIXEL_SIZE={config.pixel_size * 1000:.4f};",  # Convert m to mm, use fewer decimals
        f"DISTANCE={distance_mm:.2f};",  # Already in mm, fewer decimals
        f"WAVELENGTH={wavelength:.4f};",
        f"BEAM_CENTER_X={beam_x_mm:.2f};",
        f"BEAM_CENTER_Y={beam_y_mm:.2f};",
        f"ADXV_CENTER_X={beam_x_mm:.2f};",
        f"ADXV_CENTER_Y={beam_y_mm:.2f};",
        f"MOSFLM_CENTER_X={beam_x_mm:.2f};",
        f"MOSFLM_CENTER_Y={beam_y_mm:.2f};",
        f"DENZO_X_BEAM={beam_x_mm:.2f};",
        f"DENZO_Y_BEAM={beam_y_mm:.2f};",
        f"DIALS_ORIGIN={beam_x_mm:.2f},{beam_y_mm:.2f},0.00;",
        f"XDS_ORGX={beam_x_mm / (config.pixel_size * 1000):.1f};",
        f"XDS_ORGY={beam_y_mm / (config.pixel_size * 1000):.1f};",
        f"CLOSE_DISTANCE={distance_mm:.2f};",  # Use distance if close_distance not available
        f"PHI=0.00;",
        f"OSC_START=0.00;",
        f"OSC_RANGE=0.00;",
        f"TWOTHETA=0.00;",
