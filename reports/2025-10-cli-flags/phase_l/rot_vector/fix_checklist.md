...
| VG-1.4 | Per-φ JSON stability | Implementation Loop | ⚠️ | `per_phi_postfix/trace_py_rot_vector_per_phi.json`; **NEW: `base_vector_debug/20251122/`** | k_frac span=0.018088 (threshold <1e-6 NOT MET); 10 φ entries captured. **Attempt #101 (2025-10-07):** Regenerated traces under `base_vector_debug/20251122/`; identified C trace lacks per-φ instrumentation (`TRACE_C_PHI` lines missing). Documented in `diagnosis.md`. Drift persists (Δk=0.018088); C comparison blocked until C binary instrumented. |
