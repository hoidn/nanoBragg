...
| C2 | Implement chosen mitigation under debug prompt | [P] | Apply the selected change in the appropriate module while running inside `prompts/debug.md`. Preserve vectorization/differentiability; update tests or helper scripts as needed. **Status:** Partial — tolerance-based detection + intensity-weighted COM clustering (radius 1.5 px) yields 43/50 matches with ~1 px offsets. Work through C2a–C2c below to recover ≥95% matches before advancing. |

#### C2 Subtasks — Plateau clustering adjustments
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| C2a | Replace COM rounding with brightest-member representative per cluster | [ ] | Update `find_peaks` to return the highest-intensity coordinate from each cluster (no rounding drift). Run `KMP_DUPLICATE_LIB_OK=TRUE pytest tests/test_at_parallel_012.py::TestATParallel012ReferencePatternCorrelation::test_simple_cubic_correlation -vv` and log `n_matches`/`mean_dist` into `reports/2025-10-AT012-regression/phase_c_trials/c2a_max_intensity.log`. |
| C2b | If C2a still <48/50, evaluate float centroid output without rounding | [ ] | Allow clustered peaks to emit float coordinates (update matcher if needed) and rerun the same pytest command; capture results in `reports/2025-10-AT012-regression/phase_c_trials/c2b_float_centroid.log`. Compare offsets vs C golden peaks. |
| C2c | Document chosen mitigation and update memo | [ ] | Summarise findings in `reports/2025-10-AT012-regression/phase_c_decision.md` and link artifacts. Once ≥48/50 achieved, note final algorithm and notify DTYPE plan Phase C. |
