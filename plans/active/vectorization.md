...
- Status Snapshot (2025-12-22): Phases A-E complete - gather vectorization (commit 12742e5), batched polynomials (commit f796861), and Phase E validation (`reports/2025-10-vectorization/phase_e/summary.md`) are locked with CPU+CUDA evidence. Phase F detector absorption vectorization is next, using Phase A3 baselines as the authoritative pre-change metrics.
...
| F3 | Add regression + perf tests | [ ] | Run `scripts/benchmarks/absorption_baseline.py --repeats=200 --device cpu` now (CUDA run deferred until device-placement fix lands) to compare vs Phase A baseline (11.3 M px/s @ 256², 44.8 M px/s @ 512² CUDA). Extend `tests/test_at_abs_001.py` with parametrisation hooks if additional cases emerge; capture artifacts under `reports/2025-10-vectorization/phase_f/perf/<timestamp>/`. Target: ≤1.05× baseline regression. |
