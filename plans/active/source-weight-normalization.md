...
- Status Snapshot (2025-12-24 update): Phases A–D complete — Option B design locked with acceptance harness staged under `reports/2025-11-source-weights/phase_d/20251009T104310Z/`. Phase E1 guard shipped in commit 3140629 (UserWarning emitted when divergence flags accompany `-sourcefile`), and manual CLI checks confirm the runtime now reports `Loaded 2 sources` with no auto-generated divergence grid. TC-D2 passes via `pytest.warns(UserWarning)`, yet TC-D1/TC-D3/TC-D4 parity runs still diverge by 140–300×, indicating the remaining issue lies in the normalization pipeline (steps/fluence) rather than guard logic. Phase E2/E3 remain blocked until we capture synchronized PyTorch+C metrics plus simulator diagnostics to pinpoint the normalization fault.
...
| E2 | Extend regression tests | [B] | ⚠️ **BLOCKED** — TC-D2 conversion COMPLETE (`tests/test_cli_scaling.py:586-658`) and the CLI guard now fires correctly, but TC-D1/TC-D3/TC-D4 still fail with 140–300× divergence. Before touching tests again, capture fresh PyTorch-only diagnostics (stdout, `n_sources`, `steps`, `fluence`) under `reports/2025-11-source-weights/phase_e/<STAMP>/` so we have a trustworthy baseline feeding the normalization investigation. Failure report: `reports/2025-11-source-weights/phase_e/20251009T115838Z/summary.md`. |
| E3 | Capture parity metrics | [B] | ⚠️ **BLOCKED** — Requires synchronized PyTorch+C rerun after E2 diagnostics confirm guard behaviour. Once new CLI artifacts are in place, rebuild the C binary (if necessary), regenerate TC-D1/TC-D3 float images, and collect parity metrics plus trace inputs to isolate the normalization fault. Parity thresholds remain unmet (TC-D1: corr=-0.297, ratio=302.6×; TC-D3: corr=0.070, ratio=141.7×). |
