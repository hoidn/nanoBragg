...
Status note (2025-12-24 triage): TC-D2 conversion to `pytest.warns` remains PASSING. Fresh simulator introspection shows PyTorch honouring the sourcefile wavelength column (6.2 Å) while the C reference sticks with CLI `-lambda=0.9768 Å`, producing a ~300× intensity inflation. Steps normalization also differs (PyTorch steps=2 vs C steps=4 due to zero-weight divergence placeholders). Divergence auto-generation is already guarded; current blocker is resolving the wavelength semantics before reconciling parity metrics.
...
| E2 | Extend regression tests | [B] | ⚠️ **BLOCKED** — TC-D2 conversion COMPLETE (`tests/test_cli_scaling.py:586-658`) and the CLI guard now fires correctly, but TC-D1/TC-D3/TC-D4 still fail with 140–300× divergence. Capture two PyTorch-only runs under `reports/2025-11-source-weights/phase_e/<STAMP>/lambda_sweep/`: (a) current fixture (6.2 Å column) and (b) duplicate fixture with column 5 replaced by `9.768e-11`. Record `stdout`, `commands.txt`, `simulator_diagnostics.txt`, and metrics for both to quantify the wavelength impact before modifying tests. |
| E3 | Capture parity metrics | [B] | ⚠️ **BLOCKED** — Execute synchronized PyTorch+C reruns only after E2 confirms which wavelength semantics match nanoBragg.c. Use the validated fixture to regenerate TC-D1/TC-D3 float images, compute correlation/sum_ratio, and collect trace inputs (`trace_pixel`) for the first deviating pixel. Parity thresholds remain unmet (TC-D1: corr=-0.297, ratio=302.6×; TC-D3: corr=0.070, ratio=141.7×) until the wavelength issue is resolved. |
