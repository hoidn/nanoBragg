...
- Status Snapshot (2025-12-24 triage): Phases A–D remain complete and Option B landed (CLI wavelength override + steps guard; verified by `tests/test_at_src_003.py` and Attempt #17). Weighted-source semantics now match the spec/C reference, but Phase E is paused until we record a fresh TC-D1 single-pixel trace (PyTorch vs C) that identifies the first divergent variable; parity reruns (TC-D1/TC-D3 corr ≥ 0.999, |sum_ratio−1| ≤ 1e-3) and downstream unblocks follow that trace capture.
...
| E2a | Capture TC-D1 trace divergence | [ ] | Run the single-pixel trace harness (PyTorch + C with `-trace_pixel 158 147`) and store `py_trace.txt`, `c_trace.txt`, `diff.txt`, `commands.txt`, and `env.json` under `reports/2025-11-source-weights/phase_e/<STAMP>/trace/` so we know the first divergent variable before revisiting simulator normalization. |
