...
| E3 | Capture parity metrics | [B] | ⚠️ **BLOCKED** — Execute synchronized PyTorch+C reruns only after the CLI `-lambda` semantics fix (per Phase E2 evidence) lands and the steps denominator is reconciled. Then regenerate TC-D1/TC-D3 float images, compute correlation/sum_ratio, and collect trace inputs (`trace_pixel`) for the first deviating pixel. Current parity remains off (TC-D1: corr=-0.297, ratio=302.6×; TC-D3: corr=0.070, ratio=141.7×) until the wavelength + steps issue is resolved. |
