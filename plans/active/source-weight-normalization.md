...
| D3 | Prepare acceptance harness | [ ] | Reuse the D2 timestamp or create a fresh `<STAMP>` under `phase_d/` containing: (1) `commands.txt` with explicit C/Py CLI bundles (`-oversample 1`, divergence flags, warning expectation), (2) pytest selector `tests/test_cli_scaling.py::TestSourceWeightsDivergence -v` plus collect-only log, (3) `summary.md` scaffold listing required metrics (correlation ≥0.999, |sum_ratio−1| ≤1e-3, warning captured). These assets become the execution contract for Phase E. |
