...
- Phase M3 ✅ documentation complete (STAMP: 20251011T193829Z) — Evidence bundles captured for Sprint 1 clusters under `reports/2026-01-test-suite-triage/phase_m3/20251011T193829Z/`; follow-up Attempts #45/#47 cleared C15 mixed units and C17 polarization, and Attempt #69 verified the gradcheck compile guard. Tracker and analysis summaries now align with the Phase O baseline (STAMP 20251015T011629Z).
...
- Phase R ⏳ planning — Guarded full-suite closure pending: need a final `pytest tests/` ladder with `NANOBRAGG_DISABLE_COMPILE=1` + updated artifact bundle before archiving this initiative.
...
| P3 | Stage C18 tolerance review packet | [D] | Packet complete at `phase_p/20251015T060354Z/` with 900 s recommendation (6 % headroom). Awaiting execution of validation + tolerance implementation per §4/§5 in `c18_timing.md`. |

### Phase Q — C18 Tolerance Implementation & Closure
Goal: Implement the approved 900 s tolerance for the C18 slow gradient test, refresh evidence, and close the cluster in the ledger.
Prereqs: Phase P3 packet approved (galph 2025-10-15), validation rerun recorded under `phase_p/20251015T061848Z/`, chunk shard commands available from Phase O (lines 304-312).
Exit Criteria: `pyproject.toml` and `tests/test_gradients.py` updated with the slow-gradient timeout marker, documentation refreshed, chunk 03 rerun artifacts archived under `reports/2026-01-test-suite-triage/phase_q/<STAMP>/`, and fix_plan + remediation tracker mark C18 resolved.

| ID | Task Description | State | How/Why & Guidance (including API / document / artifact / source file references) |
| --- | --- | --- | --- |
| Q1 | Verify pytest-timeout plugin availability | [D] | **Attempt #78 (STAMP 20251015T064831Z)** installed `pytest-timeout` and recorded the preflight check under `phase_q/20251015T064831Z/precheck.md`. Manifest updates landed in Attempt #79 (`requirements.txt`, `pyproject.toml` optional test extras). |
| Q2 | Extend pytest configuration for slow gradients | [D] | **Attempt #78** extended `pyproject.toml:48-56` with the `slow_gradient` marker plus timeout defaults; diff captured in `phase_q/20251015T064831Z/config_update.md`. |
| Q3 | Annotate C18 test with slow-gradient marker | [D] | **Attempt #78** applied `@pytest.mark.slow_gradient` + `@pytest.mark.timeout(900)` at `tests/test_gradients.py:574-576`; decorator pattern matches `c18_timing.md` §5.1 Option A. |
| Q4 | Refresh documentation set | [D] | Completed 2025-10-15; docs now cite the approved 900 s allowance (`docs/development/testing_strategy.md` §4.1, `arch.md` §15, `docs/development/pytorch_runtime_checklist.md` §5) with Phase P/Q evidence references. |
| Q5 | Rerun slow-gradient validation post-change | [D] | **Attempt #79 (STAMP 20251015T071423Z)** executed `timeout 1200 env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/test_gradients.py::TestPropertyBasedGradients::test_property_gradient_stability ...` with PASS in 839.14 s; artifacts stored in `phase_q/20251015T071423Z/chunk_03/`. |
| Q6 | Ledger + tracker closure | [D] | **Attempt #80 (STAMP 20251015T071423Z)** updated `docs/fix_plan.md` Next Actions/Attempts and refreshed `remediation_tracker.md` to mark C18 resolved with the approved 900 s tolerance. Phase Q artifacts now cross-reference the summary + ledger update bundle. |

### Phase R — Guarded Full-Suite Closure
Goal: Capture a final `pytest tests/` ladder under the compile guard to prove zero active failures and stage `[TEST-SUITE-TRIAGE-001]` for archival.
...
**Status (STAMP 20251015T091543Z):** Chunk 03 rerun under the guard env reached `tests/test_gradients.py::TestPropertyBasedGradients::test_property_gradient_stability` and tripped the `@pytest.mark.timeout(900)` ceiling at 900.02 s. Evidence bundle: `reports/2026-01-test-suite-triage/phase_r/20251015T091543Z/chunks/chunk_03/`. The rerun confirms Phase P/Q runtime packets but shows zero remaining margin; increase the slow-gradient timeout (and doc references) before attempting the final ladder capture.

| ID | Task Description | State | How/Why & Guidance (including API / document / artifact / source file references) |
| --- | --- | --- | --- |
| R1 | Refresh chunk ladder & env guard | [P] | Recreate `reports/2026-01-test-suite-triage/phase_r/${STAMP}/commands.txt` from the Phase O roster, and `mkdir -p` the `chunks/chunk_##/` directories **before** running `tee` so pytest logs persist. Guard env must include `CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1`. |
| R2 | Uplift slow-gradient timeout ceiling | [ ] | Edit `tests/test_gradients.py::TestPropertyBasedGradients::test_property_gradient_stability` to use `@pytest.mark.timeout(905)` (adds ~0.5% headroom). Mirror the new ceiling in `docs/development/testing_strategy.md` §4.1, `arch.md` §15, and `docs/development/pytorch_runtime_checklist.md` §5 with citations to Phase Q (839.14 s) and Phase R (900.02 s) evidence. Capture design note under `reports/2026-01-test-suite-triage/phase_r/${STAMP}/tolerance_update/`. |
| R3 | Re-run chunk 03 with updated tolerance | [ ] | After raising the timeout + docs, rerun chunk 03 via `timeout 2400 env ... pytest -vv <chunk_03 roster> --maxfail=0 --durations=25 --junitxml chunks/chunk_03/pytest.xml`. Collect `pytest.log`, `pytest.xml`, `exit_code.txt`, and a refreshed `summary.md` quoting the 905s ceiling. |
| R4 | Aggregate results & sync ledgers | [ ] | Once chunk 03 passes, regenerate the Phase R `summary.md`, update `docs/fix_plan.md` attempts + Next Actions, and sync `reports/2026-01-test-suite-triage/phase_j/20251011T043327Z/remediation_tracker.md` to mark Phase R complete. |
