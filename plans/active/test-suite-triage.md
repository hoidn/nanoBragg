...
| ID | Task Description | State | How/Why & Guidance (including API / document / artifact / source file references) |
| --- | --- | --- | --- |
| Q1 | Verify pytest-timeout plugin availability | [ ] | Run `python -m pip show pytest-timeout`; if missing, add to `requirements.txt` + `pyproject.toml` (`[project.optional-dependencies.test]`) before proceeding. **Attempt #78 installed the plugin but did not update these manifests — finish that bookkeeping before marking Q1 done.** Record outcome in `reports/2026-01-test-suite-triage/phase_q/$STAMP/precheck.md`. |
| Q2 | Extend pytest configuration for slow gradients | [ ] | Edit `pyproject.toml:48-56` to add `slow_gradient` marker, set default `timeout` (e.g., 300 s) and `timeout_func_only=true`, and define marker docstring matching `c18_timing.md` §5.1 Option A. Capture diff snippet in `phase_q/$STAMP/config_update.md`. |
| Q3 | Annotate C18 test with slow-gradient marker | [ ] | In `tests/test_gradients.py:574`, apply `@pytest.mark.slow_gradient` (or combined marker decorator per Q2 semantics) and ensure the decorator passes `timeout=900` if configuration requires explicit per-test override. Reference `c18_timing.md` §5.1 for exact decorator pattern. |
| Q4 | Refresh documentation set | [ ] | Update `docs/development/testing_strategy.md` §4.1, `arch.md` §15, and `docs/development/pytorch_runtime_checklist.md` to record the 900 s allowance and cite `reports/2026-01-test-suite-triage/phase_p/20251015T060354Z/c18_timing.md`. Drop doc edits under `phase_q/$STAMP/docs/{testing_strategy.md,arch.md,runtime_checklist.md}`. |
| Q5 | Rerun chunk 03 shards post-change | [ ] | Reuse Phase O commands with new STAMP: `timeout 1200 env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv @reports/2026-01-test-suite-triage/phase_o/chunks/chunk_03_part3b.txt --maxfail=0 --durations=25`. Store logs/JUnit/timing under `phase_q/$STAMP/chunk_03/`. Confirm runtime ≤900 s and document in `summary.md`. |
| Q6 | Ledger + tracker closure | [ ] | Update `docs/fix_plan.md` `[TEST-SUITE-TRIAGE-001]` Attempts with Phase Q STAMP, mark C18 resolved, and refresh `reports/2026-01-test-suite-triage/phase_j/20251011T043327Z/remediation_tracker.md` entries. Include validation references (`phase_q/$STAMP/chunk_03/summary.md`) before setting State → [D]. |
