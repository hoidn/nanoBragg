...
| O5a | Stage guard bundle + STAMP scaffold | [ ] | Export `STAMP=$(date -u +%Y%m%dT%H%M%SZ)` within a **single** shell invocation and `mkdir -p reports/2026-01-test-suite-triage/phase_o/${STAMP}/{gradients,chunks/chunk_03}` before running any pytest command. Copy Attempt #69 guard artifacts (`phase_o/20251015T014403Z/grad_guard/{summary.md,exit_code.txt,pytest.xml}`) into `phase_o/${STAMP}/gradients/` so both gradcheck context and the remainder live under one timestamp. Keep `$STAMP` in scope by chaining commands with `&&` (see input handoff). |
| O5b | Regenerate selector manifests | [ ] | Derive **four** shards under `reports/2026-01-test-suite-triage/phase_o/`:<br>• `chunk_03_selectors_part1.txt` → quick CLI/parallel modules (`tests/test_at_cli_001.py`, `tests/test_at_flu_001.py`, `tests/test_at_io_004.py`, `tests/test_at_parallel_009.py`, `tests/test_at_parallel_020.py`).<br>• `chunk_03_selectors_part2.txt` → perf/pre/config sweep (`tests/test_at_perf_001.py`, `tests/test_at_pre_002.py`, `tests/test_at_sta_001.py`, `tests/test_configuration_consistency.py`, `tests/test_show_config.py`).<br>• `chunk_03_selectors_part3a.txt` → fast gradient properties (`tests/test_gradients.py::TestProperties::test_property_metric_duality`, `...::test_property_volume_consistency`, `...::test_property_hkl_consistency`).<br>• `chunk_03_selectors_part3b.txt` → slow gradient workloads (`tests/test_gradients.py::TestProperties::test_property_gradient_stability`, `tests/test_gradients.py::TestLongRunning::test_gradient_flow_simulation`).<br>Keep the original `chunk_03_selectors.txt` (all 11 modules) intact and note the new grouping rationale in `commands.txt` for traceability. |
| O5c | Run remainder — part 1 | [ ] | Command template (single line): `timeout 600 env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv @reports/2026-01-test-suite-triage/phase_o/chunk_03_selectors_part1.txt -k "not gradcheck" --maxfail=0 --durations=25 --junitxml reports/2026-01-test-suite-triage/phase_o/${STAMP}/chunks/chunk_03/pytest_part1.xml 2>&1 | tee reports/2026-01-test-suite-triage/phase_o/${STAMP}/chunks/chunk_03/pytest_part1.log`. Expect <180 s runtime; if timeout hits, capture offending module and stop. |
| O5d | Run remainder — part 2 | [ ] | Same shell/session: reuse `$STAMP` and execute the matching command with `_part2` selectors → emit `pytest_part2.{xml,log}` to the same chunk directory. Document wall time + exit code in `commands.txt`. |
| O5e | Run gradients — part 3a (fast set) | [ ] | Give the fast property tests their own 600 s budget: `timeout 600 env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv @reports/2026-01-test-suite-triage/phase_o/chunk_03_selectors_part3a.txt --maxfail=0 --durations=25 --junitxml reports/2026-01-test-suite-triage/phase_o/${STAMP}/chunks/chunk_03/pytest_part3a.xml 2>&1 | tee reports/2026-01-test-suite-triage/phase_o/${STAMP}/chunks/chunk_03/pytest_part3a.log`. Capture durations and confirm all three finish well under 600 s. |
| O5f | Run gradients — part 3b (slow set) | [ ] | Allocate a 1200 s budget to the slow property tests: `timeout 1200 env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv @reports/2026-01-test-suite-triage/phase_o/chunk_03_selectors_part3b.txt --maxfail=0 --durations=25 --junitxml reports/2026-01-test-suite-triage/phase_o/${STAMP}/chunks/chunk_03/pytest_part3b.xml 2>&1 | tee reports/2026-01-test-suite-triage/phase_o/${STAMP}/chunks/chunk_03/pytest_part3b.log`. If runtime still exceeds 1200 s, stop and record elapsed time + last visible test in `commands.txt` for supervisor escalation. |
| O5g | Aggregate chunk 03 evidence | [ ] | Summarise part1 + part2 + part3a + part3b results in `phase_o/${STAMP}/chunks/chunk_03/summary.md` (passes/fails/skips/xfails + slowest durations per shard) and regenerate `phase_o/${STAMP}/summary.md` so the baseline reflects C18 (2) + C19 (1) only. Update the helper snippet to read all **four** XML paths before computing totals. |
| O5h | Capture per-test timing + blockers | [ ] | Append a "Gradient Timing" section to the chunk summary logging wall time for `test_property_gradient_stability` and `test_gradient_flow_simulation`. Note whether throughput is acceptable (<900 s each) or if further optimisation/deferral is required under C18. |
| O6 | Refresh ledgers + clean artifacts | [ ] | After aggregation, (1) update `reports/2026-01-test-suite-triage/remediation_tracker.md` and this plan’s Status Snapshot with the new STAMP, (2) edit `docs/fix_plan.md` Next Actions 9–11 so they call out the four-part workflow, and (3) relocate the stray `phase_o/$(date -u +%Y%m%dT%H%M%SZ)/grad_guard/pytest.xml` into `phase_o/${STAMP}/gradients/` before pruning the earlier timeout directories (20251015T020729Z/20251015T023954Z/20251015T030233Z). |
