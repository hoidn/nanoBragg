...
- Phase M1 🔄 in progress — C1 (CLI fixtures) closed in Attempt #21, C3 (detector dtype conversion) closed in Attempt #22, and C4 (debug trace summary) closed in Attempt #25 (`reports/2026-01-test-suite-triage/phase_m1/20251011T164229Z/debug_trace/summary.md`); remaining Sprint 0 clusters (C5/C7) pending execution before re-attempting MOSFLM remediation.
...
| M1d | Align simulator API usage (Cluster C5) | [ ] | Update CUDA graphs fixtures to call `Simulator(crystal, detector, beam)` (see `triage_summary.md:157-182`) and capture rationale about kwarg deprecation in `phase_m1/$STAMP/simulator_api/summary.md`. Reproduce failure with `env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests/test_perf_pytorch_005_cudagraphs.py::TestCUDAGraphsCompatibility::test_basic_execution`, then rerun the full module (`pytest -v tests/test_perf_pytorch_005_cudagraphs.py`) to confirm all three tests pass. |
| M1e | Provide detectors for lattice-shape tests (Cluster C7) | [ ] | Patch fixtures referenced in `triage_summary.md:218-243` so Simulator receives a valid detector (or raise a clear error if unsupported). Validate with `env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests/test_at_str_003.py::TestAT_STR_003_LatticeShapeModels::test_gauss_shape_model` and rerun the companion test (`-k "TestAT_STR_003_LatticeShapeModels"`). Evidence lives in `phase_m1/$STAMP/shape_models/`. |
