...
| ID | Task Description | State | How/Why & Guidance (including API / document / artifact / source file references) |
| --- | --- | --- | --- |
| R1 | Refresh chunk ladder & env guard | [ ] | Export `STAMP=$(date -u +%Y%m%dT%H%M%SZ)`; duplicate chunk roster from `phase_o/20251015T043128Z/commands.txt`, pruning defunct modules noted in Attempt #46. Document the guard env (`CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1`) in `commands.txt` under `reports/2026-01-test-suite-triage/phase_r/${STAMP}/`. |
| R2 | Execute guarded pytest tests/ | [ ] | Run the 10-chunk ladder (or full `pytest tests/` if runtime permits) capturing `pytest_part##.{log,xml}` plus aggregate `pytest_full.xml`. Target runtime ≤3600 s. Verify chunk 03 gradients honour the compile guard (no donated-buffer errors). Store outputs under `phase_r/${STAMP}/chunks/chunk_##/`. |
| R3 | Aggregate results & sync ledgers | [ ] | Use the aggregation helper from Phase O (script in plan §Phase O3) to summarise totals, confirm failures=0, and write `summary.md`. Update `docs/fix_plan.md` with Attempt entry + Next Action closure and refresh `remediation_tracker.md` Executive Summary to note full-suite green status. |
