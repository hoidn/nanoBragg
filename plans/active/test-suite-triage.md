...
- Phase M ⏳ pending — 13 residual failures (clusters C2, C8, C15, C16) await Phase M3 documentation bundle + owner follow-through before the final post-fix validation rerun.
...
| M3c | Scope mixed-units zero intensity (C9) | [ ] | Promote to `[VECTOR-PARITY-001]` Tap backlog; add callchain requirement referencing `triage_summary.md:277`–`303`. Store hypotheses in `phase_m3/20251011T193829Z/mixed_units/hypotheses.md` with reproduction command (`pytest -v tests/test_at_parallel_015.py::TestATParallel015::test_mixed_units_comprehensive`). |
| M3d | Gradient guard harness integration (C2) | [ ] | Using Attempt #29 evidence + Phase M2 chunk logs, document how to ensure `NANOBRAGG_DISABLE_COMPILE=1` is exported in full-suite runs (e.g., wrapper script or harness note). Capture decisions under `phase_m3/20251011T193829Z/gradients_guard/summary.md` and tie follow-up to `[GRADIENT-FLOW-001]`. |
