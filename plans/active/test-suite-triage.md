...
| ID | Task Description | State | How/Why & Guidance (including API / document / artifact / source file references) |
| --- | --- | --- | --- |
| R1 | Refresh chunk ladder & env guard | [P] | Attempt #81 (STAMP 20251015T082955Z) exported the STAMP and executed chunks under the guard env, but skipped writing a `commands.txt` manifest. Next rerun must recreate the ladder file before invocation so the artifact set matches Phase O expectations. |
| R2 | Execute guarded pytest tests/ | [P] | Attempt #81 ran 9/10 chunks successfully; chunk 03 timed out after 1200 s while `tests/test_gradients.py::TestPropertyBasedGradients::test_property_gradient_stability` executed. Re-run chunk 03 in isolation with `timeout 2400` (40 min headroom) and, if needed, split gradients into a dedicated ladder command. Capture refreshed logs under `phase_r/${STAMP}/chunks/chunk_03/` with exit_code + junit XML. |
| R3 | Aggregate results & sync ledgers | [ ] | After chunk 03 completes, regenerate `summary.md` (hot path: reuse Attempt #81 template) and update `docs/fix_plan.md` + `remediation_tracker.md` to certify all 10 chunks ⟶ PASS/skip only. |
