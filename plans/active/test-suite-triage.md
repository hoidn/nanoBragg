...
| ID | Task Description | State | How/Why & Guidance (including API / document / artifact / source file references) |
| --- | --- | --- | --- |
| Q1 | Verify pytest-timeout plugin availability | [ ] | Run `python -m pip show pytest-timeout`; if missing, add to `requirements.txt` + `pyproject.toml` (`[project.optional-dependencies.test]`) before proceeding. Record outcome in `reports/2026-01-test-suite-triage/phase_q/$STAMP/precheck.md`. |
