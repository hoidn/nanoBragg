...
- Phase M1 🔄 in progress — C1 (CLI fixtures) closed in Attempt #21 and C3 (detector dtype conversion) closed in Attempt #22 (`reports/2026-01-test-suite-triage/phase_m1/20251011T162027Z/detector_dtype/summary.md`); remaining Sprint 0 clusters (C4/C5/C7) pending execution before re-attempting MOSFLM remediation.
...
| M1c | Guard debug trace pre-polar accumulator (Cluster C4) | [ ] | Initialise `I_before_normalization_pre_polar` and ensure `_apply_debug_output` emits the "Final intensity" summary when `-trace_pixel` is used without `-printout` (see `tests/test_debug_trace.py:108-144`). Reproduce with `env CUDA_VISIBLE_DEVICES=-1 KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests/test_debug_trace.py::TestDebugTraceFeatures::test_trace_pixel_flag` (current failure log: `reports/2026-01-test-suite-triage/phase_m1/20251011T163812Z/debug_trace/pytest_failed.log`). After the patch rerun the full module `pytest -v tests/test_debug_trace.py` and archive before/after logs under `phase_m1/$STAMP/debug_trace/`. |
