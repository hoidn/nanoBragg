...
- Status Snapshot (2025-12-08 refresh):
  ...
  - Attempt #155 (2025-10-08) removed the temporary `_run_sequential_c_parity()` fallback and restored the vectorized simulator loop, but `Crystal.apply_phi_carryover()` still clones tensors and only handles single-pixel cache hits; batched `(slow_indices, fast_indices)` support remains missing.
...
| M2g.4 | Apply cached φ vectors during physics | [ ] | Inside `_compute_physics_for_position`, invoke the new helper with batched `(slow_indices, fast_indices)` so φ=0 slices are replaced via advanced indexing (`torch.take_along_dim`/`index_put`) without cloning. Preserve spec mode behavior and cite the C snippet inline per CLAUDE Rule #11. |
