...
| M4d | Capture closure artifacts | [P] | Run `scripts/validation/compare_scaling_traces.py` with the supervisor ROI, stash outputs as `compare_scaling_traces.{txt,json}` in `fix_<timestamp>/`, update `lattice_hypotheses.md`, and refresh `sha256.txt` after adding the new files. |
