...
- Status Snapshot (2025-12-08 refresh):
  ...
  - Attempt #155 (2025-10-08) removed the temporary `_run_sequential_c_parity()` fallback and restored the vectorized simulator loop, but `Crystal.apply_phi_carryover()` still operates on a single pixel and is not invoked from the simulator path; batched `(slow_indices, fast_indices)` support and end-to-end wiring remain missing.
  - Commit f84fd5e (2025-10-08) regressed the cache helpers to scalar `slow_index`/`fast_index` signatures and added a `.item()`-based validity gate. This contradicts the Option B batch design and the "no `.item()` on differentiable tensors" rule, so restoring tensor-index APIs is now a prerequisite before wiring can continue.
...
| M2g.2b | Restore batched cache signatures | [ ] | Undo the scalar `slow_index`/`fast_index` API introduced in commit f84fd5e so `apply_phi_carryover` and `store_phi_final` consume tensor `(slow_indices, fast_indices)` batches. Replace the `.item()` validity check with tensor-native masks (e.g., `.any(dim=(-1,-2))`) to maintain differentiability and align with docs/development/pytorch_runtime_checklist.md §1. |
