...
| M2e | Validate scaling parity test | [P] | Keep `tests/test_cli_scaling_parity.py::TestScalingParity::test_I_before_scaling_matches_c` red with the current Δ≈0.209%. Capture failure logs under a new timestamp (CPU float64) prior to implementation, then rerun post-fix to prove closure and cite both logs in fix_plan. |
