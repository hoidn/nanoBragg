...
| M2 | Fix φ=0 carryover parity | [P] | Commit 3269f6d added `_phi_carryover_cache` and `test_cli_scaling_parity`, but `trace_harness.py` (20251008T081932Z) still reports F_latt=-2.380134 vs C=-2.383196653 (ΔI_before_scaling=-1968.57, -0.209% relative). Attempt #151’s `phi_carryover_diagnosis.md` shows the cache never triggers within a run and `.detach().clone()` severs gradients. Execute the M2d–M2i checklist to gather cross-pixel evidence, finalise the Option 1 pixel-indexed cache design, implement the vectorised plumbing, and validate gradients/device parity before rerunning metrics. |
...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| M2f | Finalise Option 1 data flow | [ ] | Extend `reports/2025-10-cli-flags/phase_l/scaling_validation/phi_carryover_diagnosis.md` with explicit tensor shapes, cache lifetime (initialise at run start, clear at ROI boundaries), and the call sequence that threads `(slow, fast)` indices into `Crystal.apply_phi_carryover`. Capture rationale for memory budget and cite `nanoBragg.c:2797,3044-3095`. |
