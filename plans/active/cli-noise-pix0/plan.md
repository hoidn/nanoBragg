...
| M5f | Targeted regression & CUDA smoke | [ ] | Re-run `pytest -v tests/test_cli_scaling_phi0.py` on CPU and (if CUDA available) `pytest -v -m gpu_smoke tests/test_cli_scaling_phi0.py`; store stdout/stderr logs in `option1_spec_compliance/<timestamp>/tests/` and record Attempt in docs/fix_plan.md confirming spec-mode remains green post-doc updates. |
