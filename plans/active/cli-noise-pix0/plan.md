...
2. **M2h.2–M2h.3 diagnostics** — Execute the CUDA smoke / gradcheck probes defined below once CPU evidence is archived. Record outcomes (or hardware unavailability) in `summary.md` so subsequent loops can focus on the first divergent quantity (`F_latt`, ω indexing).
...
| M2h.2 | CUDA parity probe | [P] | ⚠️ **BLOCKED** — Attempt #165 (2025-10-08). Device mismatch in debug path (`simulator.py:1516`) prevents CUDA execution. CPU fallback captured successfully (114 TRACE_PY lines, intensity 2.459e-07). Artifacts: `reports/.../20251008T162542Z_carryover_cache_validation/`. Blocker: bare `torch.tensor()` creates CPU tensors in CUDA run. Fix required: audit `_apply_debug_output()` and add `.to(device, dtype)` or infer from computation tensors. See `diagnostics.md` for stack trace and CLAUDE Rule #16 violation details. |
| M2h.3 | Gradcheck probe | [ ] | **DEFERRED** pending M2h.2 fix. Add/execute a minimal gradcheck harness (float64, 2×2 ROI, CUDA device) verifying cached tensors keep gradients; store output in `gradcheck.log`. Minimal script provided in `20251008T160802Z_carryover_cache_validation/diagnostics.md`. |
