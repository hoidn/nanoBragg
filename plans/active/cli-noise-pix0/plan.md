...
### Phase M5 — φ Rotation Realignment & Lattice Parity
Goal: Eliminate the remaining `I_before_scaling` deficit by aligning the φ rotation + reciprocal recomputation pipeline with `nanoBragg.c` (close Hypothesis H4) and restore spec-mode lattice parity before moving to CUDA/gradcheck.
Prereqs: Phase M4d artifacts archived (even with divergence), baseline traces from `reports/2025-10-cli-flags/phase_l/scaling_validation/20251008T212459Z/`, and per-φ trace harness ready (Phase M3 instrumentation design).
Exit Criteria: PyTorch per-φ trace matches C within ≤1e-6 for `ap/bp/cp`, `rot_*_star`, and `F_latt`; `scripts/validation/compare_scaling_traces.py` rerun yields `first_divergence=None`; `lattice_hypotheses.md` documents H4/H5 closure; targeted pytest (`tests/test_cli_scaling_phi0.py`) passes on CPU (and CUDA when available).

| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| M5a | Capture enhanced per-φ traces | [ ] | Extend `reports/2025-10-cli-flags/phase_l/scaling_audit/trace_harness.py` tap list (no code changes beyond instrumentation toggles) to emit `TRACE_PY_PHI` lines for `ap/bp/cp`, `rot_*_star`, and `V_actual` immediately after each φ rotation; store logs under `reports/.../phase_l/scaling_validation/fix_<timestamp>/trace_py_scaling_per_phi.log` and diff against `c_trace_scaling.log`. |
| M5b | Author rotation parity design memo | [ ] | Summarise the φ rotation contract referencing `specs/spec-a-core.md:204-240`, `docs/bugs/verified_c_bugs.md` (C-PARITY-001), and `golden_suite_generator/nanoBragg.c:3042-3095`. Capture hypotheses, required tensor ops (Rule #12/13), and device/dtype considerations in `reports/.../phase_l/scaling_validation/fix_<timestamp>/rotation_fix_design.md`. |
| M5c | Implement φ rotation + reciprocal recompute fix | [ ] | (Owner: Ralph.) Update `Crystal.get_rotated_real_vectors` / related helpers to recompute real and reciprocal vectors per φ tick using actual volume (Rule #13) and ensure batched tensor flow with CLAUDE Rule #11 docstrings. Maintain vectorization (no Python loops) and device/dtype neutrality. |
| M5d | Verify lattice parity & compare traces | [ ] | Regenerate PyTorch trace (`trace_py_scaling.log` + per-φ log) and rerun `scripts/validation/compare_scaling_traces.py` capturing `compare_scaling_traces.md`, `metrics.json`, `compare_scaling_traces.txt`, and updated `run_metadata.json`; confirm `first_divergence=None`. Update `lattice_hypotheses.md` to close H4/H5. |
| M5e | Targeted regression & CUDA smoke | [ ] | Run `pytest -v tests/test_cli_scaling_phi0.py` on CPU and (if available) `pytest -v -m gpu_smoke tests/test_cli_scaling_phi0.py` on CUDA; archive logs under the same fix timestamp and record attempt in `docs/fix_plan.md`. |
