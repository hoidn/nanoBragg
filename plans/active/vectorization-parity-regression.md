...
- Attempt #31 (20251010T113608Z) merged Tap 5 metrics into `intensity_pre_norm.md` and ranked hypotheses in `tap5_hypotheses.md`, highlighting that PyTorch treats the centre pixel HKL `(0,0,0)` as out-of-bounds while C returns 0.0. Phase E now targets Tap 5.1 (per-subpixel HKL audit) and Tap 5.2 (HKL bounds check) to confirm the suspected indexing bug before any code changes.
...
| E12 | PyTorch Tap 5.1 — per-subpixel HKL audit | [ ] | Extend `scripts/debug_pixel_trace.py` with `--taps hkl_subpixel` to log fractional HKL, rounded `(h0,k0,l0)`, `F_cell`, and `out_of_bounds` per subpixel for pixel (2048,2048) at oversample=2. Store JSON under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/py_taps/`. Reference `specs/spec-a-core.md:232-240` for the nearest-neighbour fallback contract. |
| E13 | C Tap 5.1 — mirror HKL audit | [ ] | Instrument `golden_suite_generator/nanoBragg.c` (HKL lookup block ≈ lines 3300-3355) with a `TRACE_C_TAP5_HKL` guard that emits the same schema as E12 for pixels (2048,2048) and (0,0). Archive logs under `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/c_taps/` with commands/env metadata. |
