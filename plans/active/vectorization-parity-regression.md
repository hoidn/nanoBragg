...
| E10 | Compare Tap 5 outputs & quantify delta | [ ] | Produce `reports/2026-01-vectorization-parity/phase_e0/<STAMP>/comparison/intensity_pre_norm.md` capturing side-by-side metrics (PyTorch vs C), ratio analysis, and which factors cancel. Include breakdown of `I_before_scaling` contributors (F_cell², F_latt², oversample sum) using existing trace artifacts. Update this plan + docs/fix_plan.md with findings. |
| E11 | Rank next hypotheses & instrumentation gate | [ ] | Based on E10 ratios, draft `tap5_hypotheses.md` enumerating candidate causes (e.g., subpixel loop accumulation, normalization order, background terms) with required evidence. Decide whether to pursue Tap 6 (water background) or a deeper Tap 5.1 (per-subpixel intensity audit) before any code changes. Align next inputs for Ralph in input.md. |
