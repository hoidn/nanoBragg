...
- Status Snapshot (2026-01-03): Phase A1–A3 complete (`reports/2026-01-vectorization-parity/phase_a/20251010T023622Z/` consolidated 0.999998 vs 0.721175 correlation evidence). Phase B1 reran the 4096² benchmark on HEAD and archived the failing bundle at `reports/2026-01-vectorization-parity/phase_b/20251010T030852Z/` (benchmark corr≈0.721, nb-compare corr≈0.063, sum_ratio≈225×). Phase B2 executed the authoritative parity selector (`reports/2026-01-vectorization-parity/phase_b/20251010T031841Z/`) and confirmed no active 4096² pytest cases (all variants deselected). Phase B3 validation-path memo (`reports/2026-01-vectorization-parity/phase_b/20251010T032937Z/validation_path.md`) recommends **Option A** — generate golden data and un-skip `test_high_resolution_variant`; tasks B3a–B3e below capture execution prior to ROI scope checks (B4) and trace localisation.
...
| B3a | Generate 4096² golden float image | [ ] | Run `NB_C_BIN=./golden_suite_generator/nanoBragg KMP_DUPLICATE_LIB_OK=TRUE $NB_C_BIN -lambda 0.5 -cell 100 100 100 90 90 90 -N 5 -default_F 100 -distance 500 -detpixels 4096 -pixel 0.05 -floatfile tests/golden_data/high_resolution_4096/image.bin`. Capture stdout/stderr, env metadata, and SHA under `reports/2026-01-vectorization-parity/phase_b/<STAMP>/c_golden/`. Verify binary size (64MB) and checksum before commit. |
| B3b | Document golden-data provenance | [ ] | Append command + thresholds to `tests/golden_data/README.md` and mention new asset in `docs/development/testing_strategy.md` §2.5. Store diff summary in `reports/.../summary.md` for Option A. |
| B3c | Implement ROI-based pytest body | [ ] | Replace skip stub in `tests/test_at_parallel_012.py::test_high_resolution_variant` with ROI extraction (512×512 centered window). Load new golden binary, run Simulator with identical config, assert no NaNs/Infs, compute ROI correlation (target ≥0.95), and enforce top-50 peak ≤1.0 px matches. Maintain vectorization + device/dtype neutrality. |
| B3d | Execute targeted pytest (expected FAIL) | [ ] | `KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests/test_at_parallel_012.py::TestATParallel012ReferencePatternCorrelation::test_high_resolution_variant`. Capture failing output under `reports/2026-01-vectorization-parity/phase_b/<STAMP>/pytest_highres.log` and record observed correlation/peak deltas. |
| B3e | Update ledgers with Option A progress | [ ] | Add Attempt entry to `docs/fix_plan.md` `[VECTOR-PARITY-001]` summarising B3a–B3d artifacts and failure metrics; cite this plan section for Ralph. |
