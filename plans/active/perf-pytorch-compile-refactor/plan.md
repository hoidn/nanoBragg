...
| B7 | Harden benchmark env toggles | [ ] | `scripts/benchmarks/benchmark_detailed.py` currently overwrites `NB_DISABLE_COMPILE` when `--disable-compile` is used, then unsets it regardless of prior state. Add push/pop logic (preserve caller value) and record the compile mode in each `benchmark_results.json` entry so reproducibility logs capture whether torch.compile was active. Update guidance in the script docstring to remind callers to clear `_simulator_cache` when flipping compile modes. |
