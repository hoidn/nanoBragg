...
| B7 | Harden benchmark env toggles | [ ] | Patch `scripts/benchmarks/benchmark_detailed.py` so `NB_DISABLE_COMPILE` is saved/restored (respect existing value), and have `run_pytorch_timed` emit a `compile_mode` field (`compiled`/`eager`) plus the observed env state into every `benchmark_results.json` row. Extend the CLI help to remind callers to clear `_simulator_cache` after mode flips. After the patch, rerun the regression command from `reports/benchmarks/20251001-025148/` (`--sizes 4096 --device cpu --dtype float32 --iterations 5` compiled vs `--disable-compile`) and archive both outputs/logs under `reports/benchmarks/<stamp>-env-toggle-fix/` to prove the metadata captures the mode. |
