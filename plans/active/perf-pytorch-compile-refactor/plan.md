...
| P3.0c | Validate multi-source intensity normalization parity | [P] | Code now divides by the source count (commit 2e2a6d9), but we still owe proof that weighted sources match the C baseline. Run `pytest tests/test_multi_source_integration.py::test_multi_source_intensity_normalization` on CPU (and CUDA when available), then execute `nb-compare --sourcefile reports/benchmarks/<date>-multi-source-normalization/weighted_sources.txt -- -default_F 100 -cell 100 100 100 90 90 90 -distance 100 -detpixels 128` using a 2-source file (weights 2 and 3, λ 6.2Å/8.0Å). Archive PyTorch vs C summed intensities before/after normalization in `reports/benchmarks/<date>-multi-source-normalization/summary.json` and link it in docs/fix_plan.md attempt log. |
