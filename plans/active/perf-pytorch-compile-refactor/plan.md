...
| P3.0c | Validate multi-source intensity normalization parity | [X] | ✅ COMPLETE (2025-09-30 Attempt #21). Validated weighted normalization with 2 sources (weights 2.0/3.0, λ 6.2Å/8.0Å) on CPU+CUDA. PyTorch correctly divides by `n_sources` per AT-SRC-001. C code ignores `source_I` weights (line 2616 overwrites with `I_bg`), making C↔Py weighted parity impossible. CPU/CUDA self-consistency verified (rel diff <2e-6). Semantic difference documented in `reports/benchmarks/20250930-multi-source-normalization/P3.0c_summary.md`. PyTorch behavior is spec-compliant and more flexible than C. Artifacts: `validation_results.json`, `weighted_sources.txt`, validation script. Tests: `test_multi_source_integration.py` PASSED. Decision: Accept PyTorch implementation as authoritative for weighted sources. |
...
| P3.2 | Collect benchmark data on CPU | [P] | Attempt #18 (2025-09-30) reran CPU benchmarks post P3.0/P3.0b/P3.4: warm runs 256²=4.07× faster (PASS), 512²=0.82× slower (within tolerance), 1024²=0.41× speedup (PyTorch 2.43× slower). Artifacts under `reports/benchmarks/20250930-perf-summary/cpu/`. Keep task open until weighted-source parity (P3.0c) resolves and the Phase 3 memo (P3.5) decides whether to accept the 1024² deficit or pursue Phase 4. |
