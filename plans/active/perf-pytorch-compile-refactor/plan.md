...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| P3.0 | Fix multi-source beam defaults before benchmarking | [ ] | Reproduce the crash via `env KMP_DUPLICATE_LIB_OK=TRUE python scripts/benchmarks/investigate_compile_cache.py --instances 2 --size 256 --devices cpu --sources 3`; patch `Simulator.__init__` so the `.to(...)` calls at `src/nanobrag_torch/simulator.py:431-434` never touch `None`. When callers provide only `source_directions`, seed wavelengths from `beam_config.wavelength_A` (meters) and weights from an equal-weights tensor that already lives on the target device/dtype. Archive failing run log under `reports/benchmarks/<date>-perf-summary/multi-source-before.txt` and the fixed rerun as `.../multi-source-after.txt`. |
| P3.0b | Fix multi-source polarization + weighting semantics | [ ] | Add coverage to prove each source’s Kahn factor is applied before the sum. Replace the current “use the first source” shortcut (`simulator.py:775-823`, `895-951`) with a per-source polarization pass that multiplies each source’s contribution before reduction so the result matches nanoBragg.c. Capture a parity trace under `reports/benchmarks/<date>-perf-summary/multi-source-polarization/`. |
| P3.0c | Restore multi-source intensity normalization parity | [ ] | Reproduce `tests/test_at_src_001.py::TestAT_SRC_001::test_multi_source_summed_intensity` to show the PyTorch path averages over sources because `steps` divides by Σweights (`simulator.py:680-696`). After correcting the normalization so accumulated intensities sum exactly like the C loop (`steps = sources * ...`), archive before/after totals and update `[PERF-PYTORCH-004]` observations. |
| P3.1 | Harden `benchmark_detailed.py` (zero-division guards, CLI size selection, total aggregation fix) | [ ] | Ensure warm-setup = 0 is handled; add CLI args `--sizes`, `--iterations`; stop summing boolean `cache_hit` into totals (scripts/benchmarks/benchmark_detailed.py:118); maintain repo standards. |
