...
| B6 | Warm-run reproducibility study | [P] | **REOPENED 2025-10-13 (galph loop)** after `reports/benchmarks/20251001-025148/` recorded speedup≈0.30 (PyTorch 1.77 s vs C 0.53 s). Run ten cold interpreter processes, each invoking `env KMP_DUPLICATE_LIB_OK=TRUE python scripts/benchmarks/benchmark_detailed.py --sizes 4096 --device cpu --dtype float32 --iterations 5` (one run per process via shell loop). Preserve every run’s `benchmark_results.json` and stdout under a new roll-up directory (e.g. `reports/benchmarks/<stamp>-4096-warm-repro/`) noting cache-hit flags, torch.compile mode, and system load. Analyse mean/variance vs both the prior reproducibility set (`20251001-030128-4096-warm-repro`) and the regression log (`20251001-025148`) to reconcile the discrepancy before Phase C resumes. |
| B7 | Harden benchmark env toggles | [ ] | Patch `scripts/benchmarks/benchmark_detailed.py` so `NANOBRAGG_DISABLE_COMPILE` is pushed/popped safely (preserve prior value), ensure `_simulator_cache` keys include compile mode (or flush on mode switches), and have `run_pytorch_timed` emit a `compile_mode` field (`compiled`/`eager`) plus the observed env state into every `benchmark_results.json` row. Extend the CLI help to explain the flag and the cache invalidation rule. After the patch, rerun the regression command from `reports/benchmarks/20251001-025148/` (`--sizes 4096 --device cpu --dtype float32 --iterations 5` compiled vs `--disable-compile`) and archive both outputs/logs under `reports/benchmarks/<stamp>-env-toggle-fix/` to prove the metadata captures the mode. |
...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| C1 | Measure impact of torch.compile | [ ] | Compare warm latencies with `NANOBRAGG_DISABLE_COMPILE=1` (pure eager) vs compiled path (after B7 patch, the CLI flag should set this env var and invalidate caches). Run `benchmark_detailed.py --sizes 4096 --device cpu --disable-compile` and log results with the new `compile_mode` metadata. |
