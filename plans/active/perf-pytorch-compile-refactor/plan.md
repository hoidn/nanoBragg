...
| P3.0c | Restore multi-source intensity normalization parity | [ ] | Replicate the simple_cubic multi-source run against the C harness (use `tests/test_at_src_001.py::TestAT_SRC_001::test_multi_source_summed_intensity`) to show current PyTorch path averages over sources (`steps` divides by Σweights). After adjusting the normalization to sum contributions like the C loop, archive before/after intensity totals and update `[PERF-PYTORCH-004]` observations. |
